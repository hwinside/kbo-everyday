...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("[welcome-dm]", {
      userId: user.id,
      userCreatedAt,
      cutoff: DEPLOY_DATE,
      cutoffFromEnv: !!process.env.WELCOME_DM_CUTOFF,
      systemUserId,
      comparison: userCreatedAt ? `${new Date(userCreatedAt).toISOString()} < ${new Date(DEPLOY_DATE).toISOString()} = ${new Date(userCreatedAt) < new Date(DEPLOY_DATE)}` : "no created_at",
    });
