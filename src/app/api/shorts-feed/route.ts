...
export function createShortsFeedGET(dependencies: ShortsFeedDependencies = {}) {
  ...
  return async function GET(req: NextRequest) {
    ...
    const needsPlayerTagRecheck = (data ?? []).some(
      (v) =>
        ...
        Boolean(
          v.player_id ||
          (Array.isArray(v.player_ids) && v.player_ids.length > 0),
        ),
    );
    if (
      team === "LG" ||
      ...
      playerTagChannelById = new Map(
        poolChannels.map((c) => [c.channel_id, c]),
      );
      ...
      if (
        !isTeamShortRelevant(v.title ?? "", v.team_id ?? null, {
          ...
          trustedChannel: Boolean(
            v.channel_id && trustedForLg.has(v.channel_id),
          ),
        })
      )
        return false;
...
export const GET = createShortsFeedGET();
