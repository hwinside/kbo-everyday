...
/** 유저별 최초 수신 클리핑 인트로 (하린아빠 지정 문구 — 내일 첫 발송 전원 + 이후 신규 가입 유저 커버) */
function firstIntro(teamName: string, nickname: string): string {
  return `아침에 갑작스러운 쪽지로 놀라시진 않으셨나요? 크보팬은 유저들이 등록해주신 최애팀별로 하루에 한번씩 아침 7시에 뉴스클리핑 서비스를 제공해드립니다. 앞으로 ${teamName}의 소식을 ${nickname}님께 매일 보내드릴겁니다. 혹시 수신을 원치 않으시면 마이페이지에서 설정을 OFF시키실 수 있습니다. 팀과 관련된 중요한 소식들을 놓치시지 않게끔 매일 정성을 다해 모으고 요약해서 보내드릴게요.`;
}
