...
export default function GameReviewSection({ gameId }: { gameId: string }) {
  ...
  return <section aria-label="경기 한줄평" className="mx-4 mb-5 rounded-2xl border border-border p-4">
    ...
    <dialog ref={dialog} aria-labelledby="game-review-title" onCancel={e => { e.preventDefault(); if (!busy) close(); }} className="fixed inset-x-0 bottom-0 top-auto m-0 mx-auto w-full max-w-lg max-h-[90dvh] rounded-t-3xl border border-border bg-bg-primary p-0 text-text-primary backdrop:bg-black/60">
      ...
      <div className="space-y-4 p-4 pb-[max(24px,env(safe-area-inset-bottom))]">
