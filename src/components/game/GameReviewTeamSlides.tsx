...
export default function GameReviewTeamSlides({ gameId, teamId, viewerId, request, renderCard, paused, initialPage, result }: {
  ...
  return <div ref={root} className="flex h-full min-w-0 flex-col" role="region" aria-label={`${getTeamById(teamId)?.shortName ?? "팀"} 팬 한줄평 슬라이드`}>
    ...
    <div ref={rail} className="flex flex-1 items-stretch snap-x snap-mandatory overflow-x-auto overscroll-x-contain [scrollbar-width:none]"
      ...
      {!page?.rows.length && <div className="min-h-[162px] w-full self-stretch rounded-2xl border border-border border-t-[3px] bg-bg-secondary p-2 [border-top-color:var(--review-team-color)] dark:[border-top-color:var(--review-team-color-dark)]" style={reviewTeamStyle(teamId)}>
    ...
    {!!page?.rows.length && <div className="grid shrink-0 grid-cols-[44px_minmax(0,1fr)_44px_44px] items-center text-center text-xs text-text-secondary">
    ...
    {!page?.rows.length && <div className="h-11 shrink-0" aria-hidden="true" />}
