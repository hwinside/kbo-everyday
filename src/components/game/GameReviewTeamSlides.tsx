 ...
 */
export default function GameReviewTeamSlides({ gameId, teamId, viewerId, request, renderCard, paused, initialPage, result }: {
  ...
  return <div ref={root} className="flex h-full min-w-0 flex-col" role="region" aria-label={`${getTeamById(teamId)?.shortName ?? "팀"} 팬 한줄평 슬라이드`}>
    ...
    <div ref={rail} className="flex flex-1 items-stretch snap-x snap-mandatory overflow-x-auto overscroll-x-contain [scrollbar-width:none]"
      ...
      {!page?.rows.length && <div className="min-h-[162px] w-full self-stretch rounded-2xl border border-border border-t-[3px] bg-bg-secondary p-3 [border-top-color:var(--review-team-color)] dark:[border-top-color:var(--review-team-color-dark)]" style={reviewTeamStyle(teamId)}>
        <ReviewTeamIdentity teamId={teamId} best={false} compact result={result} />
        <p className="mt-2 min-h-12 text-xs leading-5 text-text-secondary" role="status">{error || (loading || !page ? "한 줄을 불러오는 중…" : "아직 한 줄이 없어요.")}</p>
      </div>}
    </div>
    {!!page?.rows.length && <div className="mt-1 grid shrink-0 grid-cols-[44px_minmax(0,1fr)_44px_44px] items-center text-center text-xs text-text-secondary">
      ...
      <div className="flex h-11 justify-center">{!reduced && page.rows.length > 1 && <button className={control} aria-label={stopped ? "자동 넘김 시작" : "자동 넘김 정지"} onClick={() => setStopped(value => !value)}>{stopped ? <Play size={14}/> : <Pause size={14}/>}</button>}</div>
    </div>}
    {!page?.rows.length && <div className="mt-1 h-11 shrink-0" aria-hidden="true" />}
