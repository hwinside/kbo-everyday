 ...
 * 잠금화면 관련(live_activity 토글·카드 스타일·다시 표시)은 LockScreenCard로 승격(2026-07-18).
 */
export default function NotificationPrefsCard() {
  ...
  if (!user) return null;
  // live_activity(잠금화면 실시간 중계)는 LockScreenCard로 승격 — 이 카드에서 제외.
  const visibleLabels = isNative
    ? PREF_LABELS.filter(({ key }) => key !== "live_activity")
    : PREF_LABELS.filter(({ key }) => key === "news_clipping");
