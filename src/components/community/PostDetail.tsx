...
export default function PostDetail({ postId, headerTitle }: PostDetailProps) {
  ...
  return (
    <div className="min-h-screen bg-bg-primary pb-20">
      {/* Header */}
      <div className="sticky top-0 z-30 border-b bg-bg-primary" style={{ borderColor: post.team_id ? getTeamBorderColorById(post.team_id) : undefined }}>
        ...
        {(() => {
          ...
          return (
            <>
              ...
              />
              <button onClick={handleComment} disabled={!comment.trim() || !user} className="w-9 h-9 rounded-full flex items-center justify-center text-white disabled:opacity-30 transition-opacity" style={{ backgroundColor: 'var(--color-accent)' }}>
                <Send size={16} />
              </button>
            </>
          );
