...
// → WHATWG URL 로 canonicalize 후 exact key 규격 매칭 + encoded slash/dot·backslash 거부.

export const VENUE_ALLOWED_BUCKETS = new Set(["videos", "photos"]);

// 업로더 key 규격: venue-stories/{gameId}/{uuid}/{filename} (filename 에 slash 없음)
const VENUE_KEY_RE = /^venue-stories\/([A-Za-z0-9_-]+)\/([0-9a-fA-F-]{36})\/[^/]+$/;
...
export function parseStoragePublicUrl(
  ...
  // 파싱 전 raw 에 encoded slash/dot · backslash 가 있으면 거부(canonicalization 우회 차단)
  const rawRest = url.slice(prefix.length).split("?")[0].split("#")[0];
  if (/%2e|%2f|%5c|\\/i.test(rawRest)) return null;
  ...
  const path = decodeURIComponent(canonRest.slice(slash + 1));
