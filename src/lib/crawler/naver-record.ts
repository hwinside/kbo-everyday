...
import type { GameLinescore, GameLinescoreSide } from "@/lib/crawler/kbo-api";
...
export async function fetchNaverLinescore(
  ...
  try {
    ...
    return parseNaverScoreBoardLinescore(json?.result?.recordData?.scoreBoard);
  } catch {
    return null;
  }
}

/** KBO linescore 에 이닝별 값이 하나라도 있는지(전부 null 이면 이닝표 부재로 간주). */
export function hasInningBreakdown(ls: GameLinescore | null | undefined): boolean {
  return (
    !!ls &&
