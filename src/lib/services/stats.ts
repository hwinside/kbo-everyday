...
export function handleStatsGetFailure(
  ...
  if (error instanceof StatsFreshnessContractError) {
    return { body: { error: error.message, stats: [] }, status: 500, headers: NO_STORE };
  }
  ...
  if (season === "2026" || season === "current") {
    ...
    try {
      ...
      return { body: { error: (identityError as Error).message, stats: [] }, status: 500, headers: NO_STORE };
    }
    ...
    if (!validFbAt) {
      return { body: { error: "stats fallback has invalid freshness", stats: [] }, status: 500, headers: NO_STORE };
    }
    return {
      body: { stats: served, type, count: served.length, season: 2026, source: "fallback", updatedAt: validFbAt },
      headers: NO_STORE,
    };
  }
  return { body: { error: (error as Error).message, stats: [] }, status: 500, headers: NO_STORE };
}
...
export async function getStatsRouteResult(params: {
  ...
  if (season === "2025") {
    ...
    return { body: { stats, type, count: stats.length, season: 2025 }, headers: { "Cache-Control": "public, s-maxage=3600" } };
  }
  ...
  if (type === "defense") {
    ...
    return {
      body: { stats, type, count: stats.length, season: 2026, source: "static", updatedAt: statsMeta.defenseGeneratedAt },
  ...
  if (cached) return { body: cached.data, headers: statsEdgeHeaders(cached.data, cached.ageMs) };
  ...
  try {
    ...
    return { body: result, headers: statsEdgeHeaders(result, 0) };
  } catch (e: unknown) {
