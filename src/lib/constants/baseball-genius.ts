...
export const MATCH_PATH_REPLY_KIND = {
  ...
  // 로스터에 없는 실명을 받아 이름을 되물은 경로(제안할 이웃이 정확히 1명).
  name_suggest: "ack",
  // 로스터에 없는 실명인데 제안할 이웃이 없어 **모른다고만** 답한 경로.
  //
  // ⚠️ 화면 취급은 `name_suggest` 와 **같다** — 둘 다 생성 없이 코드가 쓴 단문이다.
  //   라벨을 나눈 이유는 UI 분기가 아니라 **감사 축 분리**다(삼순 2026-08-08 조건 ③):
  //     `name_suggest` → 오제안율의 분자·분모
  //     `name_unknown` → 커버리지 구멍의 크기
  //   한 칸에 두면 `name_unknown` 이 훨씬 많아 오제안율이 실제보다 낮게 보인다.
  name_unknown: "ack",
  // 답하지 못한 경로
  blocked: "unavailable",
  unsure: "unavailable",
