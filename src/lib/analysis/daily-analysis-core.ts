...
5. 경기 스코어(예: "9:7")는 해당 타이틀 보유 선수가 그 경기에서 직접 기록(승리투수·세이브·탈삼진 등)을 냈을 때만, 그 인과를 한 문장 안에 명시해서 언급하세요. 단순한 팀 승패 스코어를 근거 없이 나열하거나, 특정 선수의 수치 변동과 연결되지 않는 경기 결과 문장을 삽입하는 것은 절대 금지입니다. 연결고리가 없으면 경기 스코어를 아예 쓰지 마세요.
