...
export function getPitcherTraits(stats: {
  ...
  // 👑 에이스 (10승+ & ERA 3.5 이하)
  if (stats.wins >= 10 && era <= 3.50) traits.push({ emoji: "👑", label: "에이스", desc: `${stats.wins}승 ERA ${stats.era}`, criteria: "10승+ & ERA 3.50 이하" });

  // 🔥 탈삼진 (K/9 8.0+)
  if (k9 >= 8.0 && stats.so >= 50) traits.push({ emoji: "🔥", label: "탈삼진", desc: `K/9 ${k9.toFixed(1)}`, criteria: "K/9 8.0 이상" });

  // 🎯 제구력 (BB/9 2.0 이하 & 이닝 40+)
  if (bb9 <= 2.0 && ip >= 40) traits.push({ emoji: "🎯", label: "제구력", desc: `BB/9 ${bb9.toFixed(1)}`, criteria: "BB/9 2.0 이하 (40이닝+)" });

  // 🧊 포커페이스 (WHIP 1.10 이하)
  if (whip <= 1.10 && ip >= 50) traits.push({ emoji: "🧊", label: "포커페이스", desc: `WHIP ${stats.whip}`, criteria: "WHIP 1.10 이하 (50이닝+)" });

  // 💪 마무리 (세이브 20+)
  if (stats.saves >= 20) traits.push({ emoji: "💪", label: "마무리", desc: `${stats.saves}세이브`, criteria: "시즌 20세이브 이상" });

  // 🧱 벽 (홀드 20+)
  if (stats.holds >= 20) traits.push({ emoji: "🧱", label: "벽", desc: `${stats.holds}홀드`, criteria: "시즌 20홀드 이상" });

  // 🏔️ 이닝이터 (이닝 150+)
  if (ip >= 150) traits.push({ emoji: "🏔️", label: "이닝이터", desc: `${stats.ip}이닝`, criteria: "시즌 150이닝 이상" });

  // 😤 다승 (15승+)
  if (stats.wins >= 15) traits.push({ emoji: "😤", label: "다승", desc: `${stats.wins}승`, criteria: "시즌 15승 이상" });

  // 🛡️ 철벽 (ERA 2.50 이하 & 이닝 50+)
  if (era <= 2.50 && ip >= 50) traits.push({ emoji: "🛡️", label: "철벽", desc: `ERA ${stats.era}`, criteria: "ERA 2.50 이하 (50이닝+)" });

  // 🧨 폭탄해체반 (K/BB 4.0+)
  if (kbb >= 4.0 && stats.so >= 50) traits.push({ emoji: "🧨", label: "폭탄해체반", desc: `K/BB ${kbb.toFixed(1)}`, criteria: "K/BB 4.0 이상" });

  // 🔋 풀타임 (경기 60+, 불펜)
  if (stats.games >= 60) traits.push({ emoji: "🔋", label: "불펜철인", desc: `${stats.games}경기 등판`, criteria: "시즌 60경기 이상 등판" });

  // 🏆 완봉 (완봉 1+)
  if (stats.sho >= 1) traits.push({ emoji: "🏆", label: "완봉장인", desc: `${stats.sho}완봉`, criteria: "시즌 1완봉 이상" });
