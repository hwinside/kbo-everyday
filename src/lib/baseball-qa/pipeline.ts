...
const INJECTION_PATTERNS = [
  ...
  // 역할변경 명령 뒤에 비야구 추천·프롬프트 공개 명령을 잇는 복합 인젝션.
  // 공백/구두점으로 절 경계를 요구해 `바뀌면`·`바꾸면` 같은 조건형은 제외한다.
  /(?:너의\s+)?역할(?:을|를)?\s*(?:바꿔|교체해|변경하고|변경해)(?:\s+그리고)?[\s,.;:]+(?:주식[\s\S]{0,20}(?:추천|골라)|프롬프트(?:를)?[\s\S]{0,20}(?:보여|출력|공개|알려))/i,
  /(링크|url).*(줘|출력|보여)/i,
