...
const FORMAL_TONE_WORD_MAP = new Map<string, string>([
  ["가능해요", "가능합니다"],
  ["개념이에요", "개념입니다"],
  // ⚠️ 이 Map 에는 **의지동사(요청·명령·청유로 읽힐 수 있는 어절)**를 넣지 않는다.
  //    아래 `VOLITIONAL_MOOD_AMBIGUOUS_WORDS` 와 교집합이면 모듈 로드 시에 터진다.
  ["개념이지요", "개념입니다"],
  ["결과예요", "결과입니다"],
  ["결정돼요", "결정됩니다"],
  ["결정전이에요", "결정전입니다"],
...
/**
 * A′ ② 실 provider shadow 45런에서 tone+내용보존을 함께 통과한 **완전 어절쌍**만 등록한다.
 * 정확한 개수는 이 주석이 아니라 아래 Map 자체가 SSOT 이고, 게이트가 `.size` 를 직접 읽어
 * 검증한다 — 주석에 숫자를 복제해 두면 항상 어긋난다(2026-08-25 삼순 P2: 실제 18쌍인데
 * 주석은 19/20쌍 이었다).
 ...
 *
 * 🔴 2026-08-25 삼순 P0 으로 **`보여줘요` 도 제거**했다(20 → 19 → **현재 18쌍**). 지난번엔 "타동사가
...
/**
 * 요청·명령·청유로 읽힐 수 있는 **의지동사 어절** — 두 맵 어느 쪽에도 들어가면 안 된다.
 *
 * 🔴 2026-08-25 삼순 P0. `다시 해요.` → `다시 합니다.`, `다시 말해요.` → `다시 말합니다.`
 * 처럼 **요청문이 서술문으로 둔갑**한다. `보여요`(8/24)·`보여줘요`(8/25) 와 완전히 같은 축이다.
 *
 * 왜 반례를 하나씩 지우지 않고 부류를 통째로 닫는가 — 어절 단위 exact key 는 mood 를
 * 원리적으로 구분할 수 없다. 같은 `해요` 가 `경기를 운영해요`(서술)도 되고
 * `다시 해요`(요청)도 된다. 같은 축으로 NO-GO 가 세 번 났으면 개별 패치를 멈추고
 * **부류 자체를 금지**한다(`open_language_never_closes_with_rules`).
 *
 * 이 집합은 주석이 아니라 **모듈 로드 시점 assertion 으로 강제**된다(바로 아래 교집합 검사).
 * 새 쌍을 등록하려다 여기 걸리면 그건 버그가 아니라 계약이다.
 */
export const VOLITIONAL_MOOD_AMBIGUOUS_WORDS: ReadonlySet<string> = new Set([
  // FORMAL_TONE_WORD_MAP 에서 제거한 13개 (하다/있다 계열 의지동사)
  "해요", "말해요", "있어요", "담당해요", "등판해요", "발표해요", "발휘해요",
  "선언해요", "선정해요", "승리해요", "운영해요", "까다로워해요", "작동해요",
  // FORMAL_TONE_REWRITE_WORD_MAP 에서 제거한 6개
  "겪어요", "기려요", "받아요", "여겨요", "들어서지요", "않아요",
  // 이미 fail-close 한 과거 반례 — 재유입 방지
  "보여요", "보여줘요",
]);

// 로드 시점 fail-close — 두 맵과 의지동사 집합은 서로 배타적이어야 한다.
// 주석으로만 막으면 다음 사람이 반드시 다시 넣는다.
for (const word of VOLITIONAL_MOOD_AMBIGUOUS_WORDS) {
  if (FORMAL_TONE_WORD_MAP.has(word) || FORMAL_TONE_REWRITE_WORD_MAP.has(word)) {
    throw new Error(
      `[tone] 의지동사 어절 "${word}" 는 요청문을 서술문으로 둔갑시키므로 매핑할 수 없다`,
    );
  }
}
 ...
 * shadow에서 검토된 `FORMAL_TONE_REWRITE_WORD_MAP` 의 완전 어절쌍 중 하나여야 한다
 * (개수는 Map 이 SSOT — 주석에 복제하지 않는다). 조건 하나라도 어기면 폐기한다.
 *
