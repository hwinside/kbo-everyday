 ...
 *
 * ⚠️ 저작권(§5, §12.2-b): 나무위키는 CC BY-NC. 답변은 재서술 + canonical 출처 링크로 낸다.
 *
 * ❗️ 범위 정정(삼순 재리뷰 #4): MAX_CHUNK_CHARS는 **chunk 1건의 길이 상한**일 뿐,
 * 문서 전문이 여러 chunk로 나뉘어 결국 전부 저장되는 것을 막지는 **않는다**.
 * 이전 주석의 "문서 전문 저장을 구조적으로 차단"은 구현과 달라 정정한다.
 * 원문 저장 최소화가 목표가 되면 문서 단위 선별/retention 계약이 별도로 필요하며,
 * 이번 슬라이스 범위 밖이다(아직 확정 안 됨).
 */
