...
/** ㉠ 완전형 고유어 수사 — 뒤에 조사·어미가 붙어도 수사다. */
const NATIVE_FULL_NUMERALS = [
...
/**
 * ㉡ 관형형·충돌형 고유어 수사 — 어절 끝이거나 단위가 붙어야 수사로 본다.
 *
 * `열`(열심히·열리다)·`쉰`(쉬다의 활용)·`스무`(스무살은 수사지만 어절 경계로 충분)는
 * 완전형에 넣으면 일상어를 통째로 죽인다. 실측으로 확인한 뒤 여기로 내렸다.
 */
const NATIVE_DET_NUMERALS = [
 ...
 */
//   ⚠️ 활용형을 **열거하면** 또 새로운 활용이 우회로가 된다(삼순 2026-08-07 10라운드 실측:
//   `열이라고 해요`·`팔이죠`·`십만이라네요` 가 전부 통과했다). 서술격 조사는 활용이
//   열려 있어 닫힌 집합이 아니다 — "닫힌 집합이라 열거가 정당하다"던 9라운드 판단은 틀렸다.
//   그래서 **어간 형태**로 잡는다: 수사 뒤에 `이`+한글 이 오거나(`이라고`·`이죠`·`이었`),
//   `ㅣ` 축약형 `예요`/`였` 이 오면 서술격이다.
//   ⚠️ 다만 `이 + 아무 한글` 로 두면 **야구 어휘가 통째로 죽는다**(자체 검증 실측):
//     세`이`브 · 세`이`프 · 네`이`버   ← 앞 글자가 관형형 수사(세·네)라 수사로 오독된다.
//   그래서 `이` 뒤에 오는 **한 음절**을 서술격 활용의 첫 형태소로 제한한다.
//   활용형 전체(이에요/이라고/이라네요…)를 열거하는 것과 다르다 — 여기서 보는 것은
//   `이-` 에 결합하는 어미의 **첫 음절**이라 조합 폭발이 없다.
//   🔴 잔존 한계: 이 목록 밖 첫 음절로 활용하면 통과한다. 목록을 넓히면 세이브·세이프가
//   죽으므로, 야구 문맥에서 손실이 큰 쪽(과차단)을 피하는 선택이다.
const COPULA_STEM = "(?:이[다라래란랍야죠지네었에예구군니]|입니|예요|였)";
...
const ATTACHED_COUNTERS = [
  "연승", "연패", "이닝", "번째", "번", "회", "차", "년", "해", "승", "패", "위", "점", "루", "타", "명", "개", "골", "살",
].sort((a, b) => b.length - a.length).join("|");

/**
 * 한자 **단음절** 수사 뒤에 **붙여 써도** 수량으로 인정하는 좁은 단위.
 *
 * ⚠️ 넓은 단위를 붙여쓰기까지 허용하면 일상어가 죽는다(실측):
 *   `이해`(理解)=이+해 · `이번`=이+번 · `이승엽`=이+승 · `오타니`=오+타 · `이점`(利點)=이+점.
 * 여기 남긴 단위는 수량 외의 읽기가 사실상 없는 것들이다.
 */
const SINO_SINGLE_SAFE_COUNTERS = ["연승", "연패", "번째", "회", "년", "위", "이닝"]
  .sort((a, b) => b.length - a.length).join("|");

/**
 * **큰 자리수사**(만·억·조). 이 글자들은 앞에 낱자리가 붙거나 뒤에 한글이 이어지면
 * 사실상 수 표기로만 읽힌다 — `십만`·`만명`·`백만`.
 *
 * ⚠️ 삼순 2026-08-07 10라운드: `관중은 만명이에요`(붙여쓰기)·`십만 관중이 모였어요` 가
 *   ⑤~⑦ 을 전부 통과했다. 원인은 판정이 **단위 allowlist** 에 걸려 있었기 때문이다 —
 *   `명` 은 붙여쓰기 허용 목록에 없었고 `관중` 은 아예 목록 밖이다.
 *   단위를 더 채우는 대응은 또 유한 사전이므로, 큰 자리수사는 **단위와 무관하게**
 *   수 표기로 본다. `만루`·`억척`·`만족` 같은 일상어는 앞에 수사가 없고 뒤가 조사도
 *   아니므로 ⑨-b 의 앞 수사 요구와 ⑨-a 의 어절끝/조사 요구로 걸러진다.
 */
const LARGE_PLACES = "[만억조]";
/** 수량 표현 뒤에 오는 조사 — 어절 경계 판정 보조. */
const PARTICLE_AFTER_NUMBER = "(?:[은는이가을를에의도만과와로])";

/**
 * 큰 자리수사에 **붙여 써도** 수량으로 인정하는 단위.
 *
 * ⚠️ 여기만은 좁은 allowlist 를 쓴다. `만`·`억` 은 한국어에서 수사와 일상어가 정면으로
 *   겹치고, 그 경계를 형태만으로는 가를 수 없기 때문이다(자체 검증 실측):
 *     만명(10,000명) = 수량   ↔   만루(滿壘)·만족·만점 = 야구/일상 어휘
 *     이만수(사람 이름)       ↔   이만 원(20,000원)
 *   그래서 "붙여 쓴 큰 자리수사"는 **수량 외의 읽기가 사실상 없는 단위**일 때만 인정하고,
 *   나머지는 ⑨-a 의 어절경계·조사 축이 받는다(`십만 관중`·`백만 이상`).
 *   🔴 잔존 한계: `만타`처럼 이 목록 밖 단위와 붙여 쓰면 통과한다. 사전을 넓히면
 *   `만루` 가 죽으므로, 야구 문맥에서 손실이 큰 쪽(과차단)을 피하는 선택이다.
 */
const LARGE_PLACE_ATTACHED_UNITS = ["관중", "달러", "명", "원", "석", "표", "톤"]
  .sort((a, b) => b.length - a.length).join("|");
...
const KOREAN_NUMERIC_PATTERNS: RegExp[] = [
  ...
  new RegExp(
    `(?<![가-힣])(?:${SINO_COMPOUND}|${SINO_UNIT}|${NATIVE_DET_NUMERALS}|${NATIVE_FULL_NUMERALS})${COPULA_STEM}`,
  ),
  ...
  // ⑨-a **큰 자리수사가 어절 끝이거나 조사가 뒤따르는** 경우 — `십만 관중`·`백만 이상`.
  //     `만루`·`억척`·`만족` 은 뒤가 조사가 아니어서 안 걸린다.
  new RegExp(`(?<![가-힣])${SINO_UNIT}*${LARGE_PLACES}(?:${PARTICLE_AFTER_NUMBER}(?![가-힣])|(?![가-힣]))`),
  // ⑨-b 큰 자리수사 + 단위 **붙여쓰기** — `만명`·`억원`.
  //     ⚠️ 앞에 수사가 붙기만 하면 전부 잡는 규칙(`SINO_UNIT+ LARGE_PLACES`)은 폐기했다.
  //     그러면 `이만수`(사람 이름)가 죽는다 — 자체 검증에서 실제로 잡혔다.
  //     `십만이라네요` 같은 서술격 결합은 ⑤-b 가 이미 받으므로 여기서 중복할 필요가 없다.
  new RegExp(`(?<![가-힣])${SINO_UNIT}*${LARGE_PLACES}\\s*(?:${LARGE_PLACE_ATTACHED_UNITS})`),
];
