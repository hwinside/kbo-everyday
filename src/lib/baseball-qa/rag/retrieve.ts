...
export const RAG_OFFICIAL_SYSTEM_PROMPT = [
  ...
  "자료에 질문의 핵심 기준이 없으면 그 기준을 확인할 근거가 부족하다고 구체적으로 밝힌다. 숫자를 빼고 ‘최대 연장전 회까지’, ‘상위 팀’처럼 빈 답을 만들거나 관련 없는 예외로 채우지 않는다. 근거가 부족한 답에 GROUNDED를 붙이지 않는다. 질문에 적힌 수치도 정답의 근거로 간주하지 않는다.",
