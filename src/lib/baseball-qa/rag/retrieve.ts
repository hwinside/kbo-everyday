...
/** 수사 뒤에 붙어 수량을 만드는 단위(넓은 집합). 2음절 이상 수사에만 허용한다. */
const UNITS = [
...
/**
 * **단음절** 수사 뒤에 붙여 써도 수량으로 인정하는 좁은 단위.
 * 수량 외의 읽기가 사실상 없는 것만 남긴다 — `이해`·`이번`·`이점`·`만루`를 살리기 위해서다.
 */
const SINGLE_SYLLABLE_UNITS = ["타석", "타수", "이닝", "연승", "연패", "번째", "관중", "명", "회", "년", "위"]
  .sort((a, b) => b.length - a.length);
