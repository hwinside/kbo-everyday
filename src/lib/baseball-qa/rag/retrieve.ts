...
export const RAG_SYSTEM_PROMPT = [
  ...
  // 인물 결속 (2026-08-19 P0). 나무위키 동명이인 문서는 본문 안에서 **다른 동명이인**을
  // 자주 언급한다("같은 팀에 동명이인인 내야수 김민준이 있다"). 주인공이 못 박혀 있지 않으면
  // 모델이 그 제3자 서술을 주인공 속성으로 끌어와 포지션·소속을 뒤바꿔 답한다(실측 5/5 재현).
  "<질문 대상> 블록이 주어지면 그 사람이 이 질문의 유일한 주인공이다. 자료에 같은 이름의 다른 인물이 등장해도 주인공은 바뀌지 않는다.",
  "자료에 동명이인이 언급되면(예: '같은 팀에 동명이인인 OO OOO가 있다') 그 사람의 포지션·소속·기록·특징을 주인공의 것으로 옮겨 쓰지 않는다.",
  "주인공의 포지션은 <질문 대상> 블록의 포지션을 따른다. 자료의 포지션 서술이 그와 다르면 그 서술은 동명이인의 것으로 보고 쓰지 않는다.",
...
export interface RagRequestExtras {
  ...
  rosterBlock?: string;
  /**
   * 질문 대상(주인공) 인물 결속 블록 — kboId·이름·팀·포지션 (2026-08-19 P0).
   *
   * `rosterBlock`(질문/직전 턴이 스친 선수들의 소속 목록)과 역할이 다르다. 이건 **이 답변이
   * 누구에 대한 것인지**를 한 명으로 못 박는다. 동명이인 문서에서 제3자 서술이 주인공
...
export function buildRagLlmRequest(
  ...
  // 주인공 결속은 자료·로스터 **뒤**, 질문 바로 앞에 둔다 — 모델이 마지막으로 읽는 사실이
  // 주인공이어야 동명이인 서술에 끌려가지 않는다.
  if (extras.identityBlock) {
    sections.push(
      "<질문 대상 — 이 답변의 유일한 주인공, 동명이인과 혼동 금지>",
      extras.identityBlock,
      // 🔴 D안 (삼순 2026-08-20): 신원 첫 문장(이름·소속·포지션)은 시스템이 roster 로
      //   별도 렌더해 답변 앞에 붙인다. 모델이 신원을 재서술하면 동명이인 오귀속 통로가
      //   다시 열린다 — 생성 본문은 세부 내용만 담는다(재작성 지시와 같은 user-turn 계약 선례).
      "위 질문 대상의 소속 구단·포지션 서술은 시스템이 별도로 제공하므로 답변 본문에 쓰지 않는다. 본문은 질문이 묻는 세부 내용만 서술한다.",
      "<질문 대상 끝>",
    );
  }
