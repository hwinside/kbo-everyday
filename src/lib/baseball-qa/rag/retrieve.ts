...
/**
 * 공식 문서(tier1)가 **선수 결속 질문의 소유권을 가져올** 최대 코사인 거리.
 *
 * 🔴 왜 별도 상수인가 (삼순 2026-08-27 P0-2).
 *   종전 코드는 `ownedByEntityRag = Boolean(enabledPlayerCandidate)` 였다 — 선수 후보가
 *   잡히기만 하면 공식 RAG 를 통째로 건너뛰었다. 그래서 `문보경 보크 규칙 알려줘` ·
 *   `임찬규 투구판 이탈 규칙` 처럼 **선수는 예시 행위자일 뿐 정본이 tier1 조문**인 질문까지
 *   선수 문서로 답하게 됐다. 이건 `LG 투수 보크 규칙` 을 구단 문서가 뺏으면 안 된다는
 *   삼순 2026-08-07 P0-1(라우팅 역전)의 **선수 버전**이다.
 *
 * 🔑 판정은 룰이 아니라 근거 거리로 한다 — **이름을 지운 잔여 질문**(residual)이 tier1
 *   조문에 얼마나 가까운가. 이름을 지우는 이유는, 이름이 남아 있으면 그 선수 문서 쪽으로
 *   벡터가 끌려가 "무엇을 묻는 질문인가" 가 흐려지기 때문이다(실측: `문보경 보크 규칙` 은
 *   tier1 0.3374 / tier2 0.3242 로 선수가 이겼지만, residual `보크 규칙 알려줘` 는
 *   tier1 0.3006 / tier2 0.4531 로 뒤집힌다).
 *
 * 🔑 값의 근거 (2026-08-27 프로덕션 코퍼스 직접 질의, residual top1 tier1 거리)
 *     tier1 이 정본   0.2698 ~ 0.3540  (보크 규칙·투구판 이탈·인필드플라이·포스아웃·세이브 조건
 *                                       · 구단: 연장전 규칙 0.2852 · 투수 보크 규칙 0.2854)
 *     엔티티가 정본   0.3778 ~ 0.4817  (구단: 트윈스 역사 0.3778 · 주장 누구야 0.3979 · 왜 못해 0.4253
 *                                       · 선수: 응원가 0.4103 · 별명 0.4399 · 요즘 어때 0.4817)
 *   두 분포가 겹치지 않고 **0.3540 ~ 0.3778** 에 경계가 있다. 그 사이 `0.36` 을 택했고
 *   선수 11 + 구단 7 = **18/18** 로 갈렸다.
 *
 * 🔴 **경계 폭이 0.0238 로 좁다는 것을 그대로 밝힌다.** 아래쪽 여유 0.006 · 위쪽 0.0178 이라
 *   표본이 늘면 겹칠 수 있다. 이건 캘리브레이션이 아니라 **18건 타당성 파일럿**이고,
 *   확정 전 라벨링 세트로 재측정한다(배포 후 72시간 `distance` 분포 관측이 그 입력이다).
 *   여유가 좁을 때는 값을 바꾸기 전에 표본부터 늘린다 — 감으로 조정하면 반대편이 뚫린다.
 *
 * ⚠️ 서빙 임계(0.42)보다 **엄격**한 것이 계약이다 — 소유권을 가져오는 것은 "근거가 있다"
 *   보다 강한 주장이라 더 확실할 때만 해야 한다. 그리고 이 값은 **낮추는 방향만** 안전하다 —
 *   높이면 엔티티 문서가 정본인 질문을 규칙집이 뺏는다.
 */
export const RAG_DOCUMENT_OWNERSHIP_MAX_DISTANCE = 0.36;
/**
