 ...
 */
/**
 * ── 한글 수치 판정 문법 (사전 나열이 아니라 **형태 규칙**) ─────────────────
 *
 * 6라운드까지 계속 뚫린 이유는 매번 "빠진 단어를 채우는" 방식이었기 때문이다.
 * 그래서 **수사의 형태별 성질**로 규칙을 나눈다. 성질은 두 가지뿐이다:
 *
 *   ㉠ **완전형 수사** (`하나`·`둘`·`여덟`·`스물`·`둘째`)
 *      단독으로 수사로만 읽힌다 → **뒤에 무엇이 붙어도 수사다.**
 *      `하나뿐`·`둘째로`·`여덟은` 처럼 조사·어미가 붙어도 잡아야 하므로
 *      **뒤 경계를 요구하지 않는다.** (조사 열거는 유한 사전이라 폐기했다.)
 *
 *   ㉡ **관형형/충돌형 수사** (`한`·`두`·`세`·`네`·`열`·`쉰`·`스무`)
 *      다른 단어의 첫 음절과 자주 겹친다 → `두산`·`세계`·`네이버`·`열심히`·`쉬다(쉰)`.
 *      그래서 **어절 끝이거나 단위가 뒤따를 때만** 수사로 본다.
 *
 * 한자 수사는 음절 수로 나눈다:
 *   · 2음절 이상(`십팔`·`이십`·`십이`) → 수량 외 읽기가 드물다 → 넓은 단위 허용
 *   · 단음절(`삼`·`팔`·`영`)          → `이해`·`이번`·`오타니`처럼 일상어와 겹친다
 *       → (a) 좁은 단위면 붙여 써도 인정(`팔회`)
 *         (b) 넓은 단위면 **공백을 요구**(`삼 점` 은 수치, `이점(利點)` 은 아님)
 *
 * 이 문법의 목표는 "수치를 맞히는 것"이 아니라 **수치일 여지가 있으면 닫는 것**이다.
 * tier2 는 숫자를 아예 안 내보내는 계약이므로, 과차단의 손해(정확한 서술이
 * 안내문으로 떨어짐)가 과소차단의 손해(지어낸 수치가 출처를 달고 서빙됨)보다 작다.
 * 과차단이 어디까지 번지는지는 게이트의 **반대편 fixture** 로 관리한다.
 */

/**
 * ── 수사 파서 ───────────────────────────────────────────────────────────────
 *
 * 10라운드까지 정규식 축을 늘려 왔지만 매번 같은 방식으로 뚫렸다(삼순 11라운드:
 * `열인데요`·`팔이면`·`십만이며`·`십만이나`·`십만까지`·`백만쯤`·`만타석`).
 * 원인은 **어미와 조사를 열거**하려 한 것이다. 한국어 어미는 활용이 열려 있어
 * 열거로 닫히지 않는다.
 *
 * 그래서 판정 방향을 뒤집는다. 어절을 **앞에서부터 수사로 분해**하고,
 * 수사를 떼어낸 **나머지가 무엇인가**로 가른다:
 *
 *   ① 나머지가 없다            → 수량 (`열`, `십만`)
 *   ② 나머지가 **조사**로 시작  → 수량 (`십만이나`, `십만까지`, `백만쯤`)
 *        조사는 한국어에서 **진짜 닫힌 품사**다. 어미와 달리 새 항목이 생기지 않는다.
 *   ③ 나머지가 **서술격 어간**  → 수량 (`열인데요`, `팔이면`, `십만이며`)
 *        `이/인/입/예/였` 5형태만 본다. 그 뒤에 오는 활용은 **보지 않는다** —
 *        활용형을 열거하지 않으므로 새 활용이 우회로가 되지 않는다.
 *   ④ 나머지가 **단위**로 시작  → 수량 (`만명`, `만타석`, `십만관중`)
 *   ⑤ 그 외                     → 수량 아님 (`이승엽`, `이사회`, `세이브`, `천천히`)
 *
 * ⚠️ 남는 모호성은 **단음절 한자 수사**다. `이해`(理解)=이+해, `만루`(滿壘)=만+루 처럼
 *   수사 1글자 + 단위 1글자가 일상어와 정면으로 겹친다. 형태만으로는 가를 수 없어
 *   단음절 수사에는 **좁은 단위 집합**만 허용한다(아래 `SINGLE_SYLLABLE_UNITS`).
 *
 * ── 🔴 정책: best-effort (하린아빠 리스크 승인 2026-08-07) ──────────────────
 *
 * 이 판정기는 **완전하지 않다.** 아래 두 지점은 알고도 열어 둔 것이다:
 *   ㉮ 순음(ㅂ·ㅃ·ㅍ)으로 시작하는 서술격 활용이 있다면 통과한다.
 *      — `세이브`·`세이프`·`네이버` 를 살리기 위한 대가다(앞 글자 `세`·`네` 가 관형형 수사).
 *   ㉯ 단음절 수사 + `SINGLE_SYLLABLE_UNITS` 밖 단위를 붙여 쓰면 통과한다(`만타` 류).
 *      — `만루`·`이해`·`이번`·`이점` 을 살리기 위한 대가다.
 *
 * 완전 차단은 형태소 분석기가 필요한데, 실측 결과 이 경로에 맞지 않는다:
 *   `kiwi-nlp` 패키지는 3.9MB 로 가볍지만 **모델이 별도 84MB** 이고(모델 없이는
 *   초기화 자체가 실패한다), 이 가드는 `/api/baseball-qa` 서버리스 함수 안에서 돈다.
 *   별도 서비스로 분리하면 답변 경로에 네트워크 홉과 장애점이 하나 더 붙는다.
 *   라이선스도 LGPL-2.1-or-later 라 별도 검토가 필요하다.
 *
 * 반대로 "수사로 읽힐 수 있는 음절이 하나라도 있으면 차단" 하면 100% 보장되지만
 * `이승엽`·`이사회`·`만루` 가 전부 죽어 tier2 경로가 사실상 폐쇄된다.
 *
 * 그래서 **best-effort 로 간다**(하린아빠 승인 2026-08-07 15:06, 스레드
 * `1785918122.410299`). 근거: tier2 는 애초에 숫자를 안 내보내는 계약이고,
 * 우리가 정본을 가진 수치(순위·승패·팀타율)는 `kbo_structured` 가 먼저 가로챈다.
 * 여기서 새는 것은 **정본이 없는 서술형 숫자**뿐이다.
 *
 * ⚠️ 이 판정기를 고칠 때: 위 ㉮㉯ 를 닫으려고 단위·어미를 넓히지 마라.
 *   그 방향은 7~10라운드에 네 번 실패했고, 넓힐 때마다 야구 어휘가 죽었다.
 *   게이트의 **반대편 fixture**(`이승엽`·`만루`·`세이브` …)가 그 경계다.
 */

/** 조사 — 한국어의 **닫힌 품사**. 어미와 달리 새 항목이 생기지 않는다. */
const PARTICLES = [
  "으로써", "으로서", "이라도", "이나마", "부터", "까지", "처럼", "보다", "마다", "조차", "밖에",
  "이나", "이란", "이든", "이야", "으로", "에서", "에게", "한테", "커녕", "대로", "만큼", "쯤",
  "뿐", "은", "는", "이", "가", "을", "를", "에", "의", "도", "만", "과", "와", "로", "랑", "께",
].sort((a, b) => b.length - a.length);

/**
 * 서술격 조사 `이다` 의 **어간 형태**. 5개뿐이고 늘어나지 않는다.
 * 뒤에 오는 활용(`~라고`·`~면`·`~며`·`~죠`·`~데요`)은 **보지 않는다** —
 * 활용을 열거하지 않는 것이 이 설계의 핵심이다.
 */
const COPULA_STEMS = ["이", "인", "입", "예", "였"];

/**
 * 수사 뒤에 붙어 수량을 만드는 단위(넓은 집합). 2음절 이상 수사 또는 **띄어 쓴** 형태에 쓴다.
 *
 * 야구 수량명사(`경기`·`안타`·`홈런`·`타석`)는 corpus 실측상 **띄어 쓴다** —
 * `만 경기` 504 · `만 안타` 245 · `만 홈런` 159 · `만 타석` 71 이고, `만경기`류 붙여쓰기는 0건.
 * 그래서 이 단위들은 여기(띄어쓰기 경로)에만 두고 붙여쓰기 목록에는 넣지 않는다.
 */
const UNITS = [
  "타석", "타수", "이닝", "연승", "연패", "번째", "관중", "달러", "경기", "안타", "홈런",
  "번", "회", "차", "년", "해", "승", "패", "위", "점", "루", "타", "명", "개", "골", "살", "원", "석", "표", "톤", "배", "분", "초", "일", "월", "주",
].sort((a, b) => b.length - a.length);

/**
 * **단음절** 수사 뒤에 **붙여 써도** 수량으로 인정하는 단위.
 *
 * ⚠️ 이 목록은 추정이 아니라 **실 corpus 빈도**로 정했다(2026-08-07, 하린아빠 지적).
 *   나무위키 corpus 전량(4,464문서)에서 `수사+단위` 붙여쓰기/띄어쓰기 실측:
 *     타석  붙임 5   ↔ 띄움 515      경기 붙임 511 ↔ 띄움 20,800
 *     타수  붙임 2   ↔ 띄움 1        안타 붙임 11  ↔ 띄움 4,276
 *   그리고 `만경기`·`만안타`·`만홈런`·`만타석`·`만타수`·`만이닝` 은 **전체 0건**이다.
 *   한국어는 같은 뜻을 `만 경기`(504)·`만 안타`(245)·`만 홈런`(159) 처럼 **띄어 쓴다.**
 *
 *   ⚠️ 더구나 위 "붙여쓰기" 수치는 대부분 **가짜 매칭**이었다(수사가 아닌 글자가 우연히 겹침):
 *     루 25,162 중 `만루`(滿壘) 23,002 · 경기 511 중 `구경기`(야구경기) 262 · `천경기` 151
 *   즉 야구 수량명사의 붙여쓰기는 실사용이 없을 뿐 아니라, 넣으면 오탐만 늘린다.
 *
 *   `만타석` 을 여기 넣었던 것은 삼순 지적을 **실사용 확인 없이** 사전에 반영한 내 잘못이다
 *   (하린아빠 2026-08-07: "만경기 만홈런 만안타 라는 말은 처음 들어봄. 안 쓰는 말이야").
 *   띄어 쓴 형태(`만 타석`)는 이 목록이 아니라 **다음 어절을 보는 경로**가 받는다.
 *
 *   남긴 단위는 붙여쓰기 실사용이 확인되고 동음이의 위험이 없는 것들이다:
 *     회 3,103 · 위 3,099 · 명 7,207 · 년 199 · 이닝 31 · 번째 9 · 관중 6(`백만관중`)
 *     연승·연패는 빈도가 낮지만(`삼연패` 2) 다른 뜻으로 읽힐 여지가 없어 유지한다.
 *
 * 붙여쓰기를 좁게 유지하는 본래 목적은 그대로다 — `이해`(理解)·`이번`·`이점`·`만루`가
 * 수량으로 오독되지 않게 하는 것. 빈도가 사실상 0 인 단위를 넣으면 그 목적만 흐려진다.
 */
const SINGLE_SYLLABLE_UNITS = ["번째", "관중", "이닝", "연승", "연패", "명", "회", "년", "위"]
  .sort((a, b) => b.length - a.length);

/** 완전형 고유어 수사 — 단독으로 수사로만 읽힌다. */
const NATIVE_FULL_LIST = [
  "하나", "둘", "셋", "넷", "다섯", "여섯", "일곱", "여덟", "아홉",
  "스물", "서른", "마흔", "예순", "일흔", "여든", "아흔",
  "첫째", "둘째", "셋째", "넷째", "다섯째", "여섯째", "일곱째", "여덟째", "아홉째", "열째",
].sort((a, b) => b.length - a.length);

/** 관형형·충돌형 고유어 수사 — 일상어 첫 음절과 겹친다(`두산`·`세계`·`열심히`). */
const NATIVE_DET_LIST = ["한", "두", "세", "서", "석", "네", "너", "넉", "열", "쉰", "스무"]
  .sort((a, b) => b.length - a.length);

const SINO_DIGIT_CHARS = "영일이삼사오육칠팔구";
const SINO_PLACE_CHARS = "십백천만억조";

/** 순음 초성(ㅂ·ㅃ·ㅍ) — 서술격 어미에는 거의 오지 않는다. `세이브`·`네이버` 방어용. */
function hasLabialOnset(ch: string): boolean {
  const code = ch.charCodeAt(0) - 0xac00;
  if (code < 0 || code > 11171) return false;
  return [7, 8, 17].includes(Math.floor(code / 588));
}

/** 어절 앞에서 수사로 읽을 수 있는 **모든 길이**를 길이 내림차순으로 돌려준다. */
function numeralPrefixes(token: string): { length: number; syllables: number; native: "full" | "det" | null }[] {
  const out: { length: number; syllables: number; native: "full" | "det" | null }[] = [];
  // 고유어 수사 연속 — 각 단계를 후보로 남긴다(`열두` → `열두`, `열`).
  let i = 0;
  let lastKind: "full" | "det" | null = null;
  for (;;) {
    const full = NATIVE_FULL_LIST.find((w) => token.startsWith(w, i));
    const det = full ? null : NATIVE_DET_LIST.find((w) => token.startsWith(w, i));
    if (!full && !det) break;
    i += (full ?? det!).length;
    lastKind = full ? "full" : "det";
    out.push({ length: i, syllables: i, native: lastKind });
  }
  // 한자 수사 연속 — 자리수사가 없는 2음절 이상 나열은 수 표기가 아니다(`이사`·`사오`).
  let j = 0;
  let places = 0;
  const sino: { length: number; syllables: number; native: null }[] = [];
  while (j < token.length) {
    const ch = token[j];
    if (SINO_PLACE_CHARS.includes(ch)) places += 1;
    else if (!SINO_DIGIT_CHARS.includes(ch)) break;
    j += 1;
    if (j === 1 || places > 0) sino.push({ length: j, syllables: j, native: null });
  }
  return [...out, ...sino].sort((a, b) => b.length - a.length);
}

/**
 * 한 어절이 수량 표현인가.
 *
 * ⚠️ 수사 분해는 **백트래킹**해야 한다. 가장 긴 읽기만 보면 서술격 조사의 `이` 를
 *   한자 수사로 먹어버린다 — `십만이며` 를 `십만이`+`며` 로 읽고 놓쳤다(자체 실측).
 */
function tokenIsNumeric(token: string, nextToken: string): boolean {
  for (const head of numeralPrefixes(token)) {
    const rest = token.slice(head.length);

    if (rest.length === 0) {
      // 고유어 수사는 단독으로 수사로만 읽힌다.
      if (head.native !== null) return true;
      // 한자 수사 단독은 고유명사와 겹친다(`이천` 베어스 파크·`이만`수).
      // 뒤 어절이 단위면 수량이다(`십만 관중`·`삼 점`·`삼십사 년`).
      if (UNITS.some((u) => nextToken.startsWith(u))) return true;
      continue;
    }

    // 근사 수량 접미 `여` — `십여 회`·`이십여 년`.
    if (rest.startsWith("여")) return true;

    // 서술격 조사 어간. 뒤에 오는 활용은 보지 않는다(활용을 열거하지 않는 것이 핵심).
    if (COPULA_STEMS.includes(rest[0])) {
      const labialAfterI = rest[0] === "이" && rest.length >= 2 && hasLabialOnset(rest[1]);
      // `세이브`·`네이버`: 단음절 관형형 수사 + `이` + 순음. 서술격 어미에 순음은 거의 없다.
      // 🔴 잔존 한계 ㉮: 순음으로 시작하는 서술격 활용이 있다면 통과한다.
      //    best-effort 정책으로 **알고도 연 것**이다(파일 상단 §정책, 하린아빠 승인 2026-08-07).
      //    닫으려고 이 조건을 지우면 `세이브`·`세이프`·`네이버` 가 과차단된다(mutation M35 가 RED).
      if (!(head.syllables === 1 && labialAfterI)) return true;
    }

    // 단위 결합. 단음절 수사에는 좁은 단위만 허용한다 — `이해`·`이번`·`이점`·`만루`를 살린다.
    // 🔴 잔존 한계 ㉯: 좁은 목록 밖 단위를 붙여 쓰면 통과한다(`만타` 류).
    //    best-effort 정책으로 알고도 연 것이다. 넓히면 `이승엽` 이 과차단된다(mutation M36 가 RED).
    ...
    //   완전형 고유어(`하나뿐`·`둘째로`)와 2음절 이상(`십만까지`·`백만쯤`)만 받는다.
    if ((head.syllables >= 2 || head.native === "full") && PARTICLES.some((pt) => rest.startsWith(pt))) return true;
  }
  return false;
}
...
export function hasKoreanNumericExpression(answer: string): boolean {
  // 어절 단위로 자른 뒤 각 어절을 파서에 태운다. 정규식 축 열거가 아니다.
  // 어절 경계에 붙는 문장부호·괄호는 떼어낸다(`열이에요.` → `열이에요`).
  return answer
    .split(/[\s]+/)
    .map((token) => token.replace(/^[^가-힣]+|[^가-힣]+$/g, ""))
    .filter((token) => token.length > 0)
    .some((token, index, all) => tokenIsNumeric(token, all[index + 1] ?? ""))
    // 서수 접두 `첫` 은 수사 분해 대상이 아니라 접두사라 따로 본다.
    || /(?<![가-힣])첫/.test(answer);
}
