...
/** 한자 수사. 단독 1음절은 조사·지시사·고유명사와 겹치므로 결합 규칙을 따로 둔다. */
const SINO_NUMERAL_WORDS = ["일", "이", "삼", "사", "오", "육", "칠", "팔", "구", "십", "백", "천"].join("|");

/** 수사에 붙어 수량을 만드는 단위. `단`(구단)·`장`(구장) 등 구단 서술 상용 음절은 제외한다. */
const ATTACHED_COUNTERS = [
  "연승", "연패", "이닝", "번째", "번", "회", "차", "년", "해", "승", "패", "위", "점", "루", "타", "명", "개",
].sort((a, b) => b.length - a.length).join("|");

/**
 * 한자 **단음절** 수사 뒤에만 허용하는 단위 (좁은 집합).
 *
 * ⚠️ 전체 단위를 그대로 쓰면 일상어가 통째로 죽는다(자체 검증 실측):
 *   · `이해` (理解)  = 이+해   · `이승엽`   = 이+승
 *   · `이번 시즌`    = 이+번   · `오타`     = 오+타
 * 단음절 수사는 한국어에서 다른 형태소와 너무 자주 겹치므로, 수량 해석 외의
 * 읽기가 사실상 없는 단위만 남긴다. `십이 번` 처럼 **2음절 이상** 조합은
 * 아래 일반 규칙이 잡으므로 여기서 빠져도 누락되지 않는다.
 */
const SINO_SINGLE_SAFE_COUNTERS = ["연승", "연패", "번째", "회", "년", "위", "이닝"]
  .sort((a, b) => b.length - a.length).join("|");

const KOREAN_NUMERIC_PATTERNS: RegExp[] = [
  // ① 고유어 수사 — **연속 조합**을 한 덩어리로 본다.
  //    `+` 가 없으면 `열두`·`스물두`·`열한` 이 내부 한글 경계에 막혀 전부 빠져나간다
  //    (삼순 2026-08-07 6라운드 지적: `열두 번`·`열한 번째`·`스무 번` 미검출).
  new RegExp(`(?<![가-힣])(?:${NATIVE_NUMERAL_WORDS})+(?![가-힣])`),
  // ② 서수 접두 `첫` — 뒤 경계를 요구하지 않는다(`첫해`·`첫째`·`첫 우승`).
  /(?<![가-힣])첫/,
  // ③ **고유어 수사** 또는 **2음절 이상 한자 수사** + [공백] + 단위.
  //    ⚠️ 뒤 경계를 **요구하지 않는다**. 요구하면 `팔회라고 알려져요` 처럼 조사 사전에
  //    없는 어미가 붙는 순간 빠져나간다(삼순 6라운드 지적). 조사 열거는 그 자체가
  //    다시 유한 사전이므로 폐기했다.
  //    ⚠️ 한자 **단음절**은 여기서 제외한다. 넓은 단위 집합과 붙으면 일상어가 통째로
  //    죽는다(자체 검증 실측): `이해`=이+해 · `이번`=이+번 · `이승엽`=이+승 · `오타니`=오+타.
  //    단음절은 ④ 가 좁은 단위로 받는다.
  new RegExp(
    `(?<![가-힣])(?:(?:${NATIVE_NUMERAL_WORDS})+|(?:${SINO_NUMERAL_WORDS}){2,})\\s*(?:${ATTACHED_COUNTERS})`,
  ),
  // ④ 한자 **단음절** 수사 + 좁은 단위(`팔회`·`십년`·`삼연승`). 수량 외의 읽기가
  //    사실상 없는 단위만 받아 ③ 의 오탐을 피한다. 2음절 이상 조합은 ③ 이 담당한다.
