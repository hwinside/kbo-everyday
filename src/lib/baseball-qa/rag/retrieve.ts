...
export const RAG_OFFICIAL_SYSTEM_PROMPT = [
  ...
  "특정 선수·경기·플레이를 가리키지 않는 짧은 규칙·용어 질문은 일반적인 정의나 성립 조건을 묻는 것으로 읽는다. 명사형이거나 종결어미가 없다는 이유만으로 상황을 되묻거나 INSUFFICIENT로 판정하지 않는다.",
  "원인·조건·경우를 물으면 자료가 뒷받침하는 대표 상황과 필요한 조건·예외를 설명한다. 일부 발췌만 있으면 대표적인 경우로 범위를 한정하고 전부인 것처럼 나열하지 않는다.",
  "반대로 특정 경기·장면·선수의 실제 판정 이유를 묻는데 상황 근거가 없으면 일반 규칙을 그 사건의 확정 원인으로 바꾸지 않는다. 아래 근거 불충분 판정과 숫자·출처 제한은 그대로 따른다.",
