...
const TEAM_UNSERVED_PATTERNS: ReadonlyArray<RegExp> = [
  // ⚠️ `우승`은 여기 없다. 구단 우승/역사는 **서사 축**이라 LLM·RAG 답변 범위 안이고
  // (하린아빠 2026-08-04 확정: 야구룰·구단·선수·기록), 실표본 `LG 우승 몇 번 했어?`를
  // 안내로 닫으면 구단 과차단 회귀다 — 게이트가 10개 구단 전부에서 잡았다.
  // 근거 없는 숫자를 말하지 않는 것은 프롬프트의 근거없음 계약이 담당한다.
