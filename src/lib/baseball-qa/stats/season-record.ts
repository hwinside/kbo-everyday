...
function scanTemporalRefs(question: string): TemporalScan {
  ...
  if (/데뷔|입단/.test(rest)) {
    if (/(?:데뷔|입단)[^]{0,8}?\d+(?:년|시즌)/.test(rest)) {
      debutScope = "other";
    } else if (/(?:데뷔|입단)[^]{0,12}?(?:첫(?:번째|째)?|(?:두|둘|세|셋|네|넷|다섯|여섯|일곱|여덟|아홉|열)(?:번째|째)|\d{1,2}번째)(?:해|시즌|경기|타석|등판|년도?)/.test(rest)) {
      // `입단 후 첫해 타율`·`데뷔 후 첫 번째 시즌`·`데뷔 후 두 번째 시즌`·`입단 후
      // 둘째 해`·`데뷔 후 2번째 시즌` (삼순 6·7·8차) — 연결어 `후` 가 있어도 뒤에
      // **단일 서수 selector**(첫/첫째/첫 번째·두 번째~열 번째·둘째~열째·N번째)가 오면
      // 특정 한 시즌 질의다. 첫째뿐 아니라 모든 단일 서수를 full-origin 판정 **전에**
      // other 로 닫아야 career total 로 축소되지 않는다(fail-close). bounded(숫자)와
      // 같은 우선순위 층. 단 `데뷔 후 두 시즌`(서수 표지 없는 수사)은 span 질의라
      // 서수 표지(번째/째)를 요구한다 — 첫(해/시즌…)만 표지 생략 허용.
      debutScope = "other";
