...
/**
 * **카운트 명사** — 그 자체로는 값 요구가 아니다(명사일 뿐).
 *
 * 🔴 `수`·`기록`·`성적` 은 뒤에 무엇이 붙느냐로 뜻이 갈린다:
 *   · `기록이 뭐야?`   → 값 요구  (삼순 지적: 종전엔 `뭐` 가 정의로 이겨 샜다)
 *   · `수는 어떻게 세?` → 정의     (삼순 지적: 종전엔 `수는` 이 값으로 이겨 막았다)
 *   그래서 **명사 + 술어 문법**으로 묶고, 단어 포함으로 판정하지 않는다.
 *
 * ⚠️ `수` 는 앞에 공백을 요구한다 — 안 그러면 `타수`·`투수`·`점수` 에서 잡힌다.
 */
const UNTRUSTED_COUNT_NOUN = /(?:^|\s)(?:개수|횟수|기록|성적)|\s수(?:는|가|를|은|도)?(?:\s|$)/;

/** 카운트 명사 뒤에 붙어 **값을 달라**는 뜻이 되는 술어. */
const UNTRUSTED_COUNT_VALUE_PREDICATE = /뭐|뭔|무엇|얼마|어때|어떻게\s*돼|알려|보여|말해/;
...
export function untrustedValueAsk(question: string, playerBound: boolean): boolean {
  ...
  // ② 카운트 명사 + 값 술어 — `기록이 뭐야?` · `성적 알려줘`.
  //    명사만으로도, 술어만으로도 값 요구가 아니다. 둘이 함께여야 한다.
  if (UNTRUSTED_COUNT_NOUN.test(spaced) && UNTRUSTED_COUNT_VALUE_PREDICATE.test(spaced)) {
    return true;
  }
