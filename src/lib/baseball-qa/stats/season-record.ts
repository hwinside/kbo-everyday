...
function scanTemporalRefs(question: string): TemporalScan {
  ...
  if (/데뷔|입단/.test(rest)) {
    if (/(?:데뷔|입단)[^]{0,8}?\d+(?:년|시즌)/.test(rest)) {
      debutScope = "other";
    } else if (/(?:데뷔|입단)[^]{0,10}?첫(?:번째)?(?:해|시즌|경기|타석|등판|년도?)/.test(rest)) {
      // `입단 후 첫해 타율`·`데뷔 후 첫 시즌 홈런`·`데뷔 후 첫 번째 시즌` (삼순 6·7차) —
      // 연결어 `후` 가 있어도 뒤에 single 마커(첫해·첫 시즌·첫 번째 시즌/해/년…)가 오면
      // 단일 데뷔시즌 질의다. 서수 동치(`첫 번째`)는 `번째` 를 optional 로 흡수한다.
      // full-origin 판정 **전에** 닫아야 career total 로 축소되지 않는다. bounded(숫자)와
      // 같은 우선순위 층.
      debutScope = "other";
