...
export type DefinitionExplanationApproach = "situation" | "conditions" | "contrast";

/** Application-resolved topic/presentation only. Never store model prose or repair data. */
export interface StatDefinitionContext {
  version: 1;
  terms: string[];
  period: DefinitionPeriodScope;
  explanationApproach?: DefinitionExplanationApproach;
}

const PERIODS: readonly unknown[] = ["season", "career", "mixed", "unspecified"];
const TERMS = new Set<string>(KBO_OFFICIAL_METRIC_TERMS);
const APPROACHES: readonly unknown[] = ["situation", "conditions", "contrast"];
...
export function readStatDefinitionContext(value: unknown): StatDefinitionContext | undefined {
  ...
  if (row.explanationApproach !== undefined && !APPROACHES.includes(row.explanationApproach)) return undefined;
  return { version: 1, terms: [...row.terms] as string[], period: row.period as DefinitionPeriodScope,
    ...(row.explanationApproach ? { explanationApproach: row.explanationApproach as DefinitionExplanationApproach } : {}) };
}
