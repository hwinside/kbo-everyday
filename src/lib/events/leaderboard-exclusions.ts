 ...
 * - 리더보드 쿼리 레벨에서 필터
 *
 * 2026-06-09 하린아빠 확정: 운영자/테스트 계정 제외 해제 → 명예의 전당에 함께 노출.
 * 봇(움짤콜렉터)만 제외 유지 (자동 발행이라 순위 왜곡 방지).
 * 해제된 계정: 하린아빠/정배현우/크보팬 운영팀/하린엄마/QA테스터/윤연률/김현우.
 * (프로즌 event_leaderboard_snapshot 은 이벤트 최종결과라 불변 — 본 변경은 라이브 뷰만 영향)
 *
 * 변경 절차:
 * - 봇/자동화 계정 신규 발생 시 이 배열에 UUID 추가 + 주석 업데이트
...
export const LEADERBOARD_INTERNAL_USER_IDS: readonly string[] = [
  '75ee70e1-d5d1-4cbe-a2f7-a937e717437c', // 움짤콜렉터 (봇, is_bot) · 2026-06-01 제외 확정, 2026-06-09 단독 잔류
] as const
