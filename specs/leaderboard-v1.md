...
### 2.1 집계 기간 (2026-04-20 하린아빠 확정)
- **두 트랙 모두 lifetime 누적** — 기간 필터 없음
- 2026-04-20 ~ 2026-05-31은 **공지상 운영 프레임/상금 지급 대상 선정 기간**일 뿐, 집계는 가입 이후 모든 활동 포함
...
### 4.1 테이블 (신규 or 뷰)
- **뉴로 구현 확정**:
  - `v_leaderboard_invite`: `invitations.activated_at IS NOT NULL` lifetime 집계 (기간 필터 없음)
  - `v_leaderboard_writing`: `chat_messages` + `comments` + `posts` lifetime 집계 (일일 측 + 합산 측 유지)
- 캐싱: `/api/leaderboard/{track}?limit=100` Next.js ISR `s-maxage=60, stale-while-revalidate=120`
- 이벤트 기간 종료 후 snapshot 테이블로 동결 검토 (`leaderboard_final_invite`, `leaderboard_final_writing`)

### 4.2 포인트 계산 쿼리 (실제 적용본)

실구현은 `supabase/migrations/20260420_leaderboard_views.sql` 참조. 핵심 구조:

```sql
-- 글쓰기 트랙: lifetime 누적 집계 (2026-04-20 하린아빠 확정)
-- 기간 필터 없음 — 가입 이후 모든 활동 포함
WITH chat_daily AS (
  SELECT user_id,
...
-- post_general_daily, post_photo_daily 유사 (content_type 구분)
combined AS (
  SELECT user_id, day, pts FROM chat_daily
  UNION ALL SELECT user_id, day, pts FROM comment_daily
  -- ...
)
SELECT user_id, SUM(LEAST(daily_total, 150)) AS total_points
FROM (SELECT user_id, day, SUM(pts) AS daily_total FROM combined GROUP BY user_id, day) d
GROUP BY user_id
ORDER BY total_points DESC;
```

> 일일 측(활동별) + 일일 합산 측(150pt) 이중 적용. 날짜 경계는 KST 기준.
...
- [ ] lifetime 누적 집계 확인 (4/19 이전 활동도 집계됨 — 2026-04-20 정책 변경)
