...
- 같은 `article_key` 요청은 기존 `post_id`를 반환하며 최초 저장된 메타데이터를 변경하지 않는다. 공개 ensure 요청으로 운영자 링크/제목을 덮어쓸 수 없어야 한다.
...
- 서버 댓글 수는 `is_hidden=true` 댓글을 제외해 신고 블라인드 후 댓글 시트와 홈 카드가 같은 수치를 표시한다.
- 블라인드된 루트 댓글에 달린 답글도 시트에서 orphan으로 보이지 않으므로 서버 표시 수에서 함께 제외한다.
- 세 번째 신고로 댓글이 블라인드되면 열린 댓글 시트에서 즉시 제거하고 카드 수를 재동기화한다.
- 댓글 시트에서 여는 신고 시트는 댓글 시트(`z-index: 9999`)보다 높은 레이어에 표시한다.
