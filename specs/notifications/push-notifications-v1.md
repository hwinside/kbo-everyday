...
| G2 | 알림 7종(경기 시작·종료 / 내 팀 득점 / 최애선수 활약·삼진 / 최애선수 관련글 / 댓글·답글 / 쪽지) 자동 트리거 | 트리거별 E2E |
...
| 쪽지 수신 | `dm_messages` insert | 수신자 | 쪽지함 대화 | 유저 DM + 운영팀 공지 둘 다 포함(확정) |
...
- **✅ 확정: A안 — FCM 단일 게이트웨이 (2026-06-10, 하린아빠 "a로 가자" + 삼순 GO)**. iOS/Android 모두 *FCM token*으로 통일.
  - iOS: APNs 키/인증서를 **Firebase 콘솔에 연결**하고, 앱에서는 **Firebase Messaging SDK 기반 `@capacitor-firebase/messaging`**으로 *FCM token*을 받아 저장. ⚠️ `@capacitor/push-notifications` 단독은 iOS에서 raw APNs token을 반환 → FCM 게이트웨이와 불일치(iOS 수신 실패 위험)라 **iOS 토큰 확보는 FCM token으로 명시**.
  - Android: `@capacitor-firebase/messaging` FCM token 직접.
  - 서버 전송 = FCM Admin SDK 1벌. Web은 기존 `web-push`(VAPID) 유지.
- **디바이스 토큰 테이블 신규**: `device_push_tokens`(user_id, platform[ios|android], fcm_token, created_at, last_seen) — 네이티브 FCM token 전용. 웹은 기존 `push_subscriptions`(VAPID) 유지(분리).
...
- **S1 토대**: `@capacitor-firebase/messaging`(Firebase Messaging) 설치 + Firebase 프로젝트/APNs 키 연결 + FCM Admin SDK 서버 전송 + `device_push_tokens`(FCM token) 테이블 + 토큰 등록 API + 권한 요청 UX. **성공 기준 = iOS/Android 실기기에서 어드민 수동 테스트 푸시 1건 도달**(삼순 명시).
