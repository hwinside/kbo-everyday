...
- **알림 설정 테이블 신규**: `notification_prefs`(user_id, game_start, game_end, my_team_score, comment_reply, dm, quiet_hours) — 종류별 토글.
...
- **S3 댓글/답글 + 쪽지**: `comments`/`dm_messages` insert 트리거 → 디스패처. 딥링크 연결. (앱 내부 이벤트라 트리거 단순)
- **S4 경기 시작/종료**: game-events 상태 전이 → 디스패처. 최애팀 매칭.
- **S5 내 팀 득점**: celebration 득점 이벤트 → 디스패처 + **rate limit/묶음**. (가장 폭탄 위험 높음 → 마지막 + 신중)
