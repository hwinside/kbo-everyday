...
1. **종료 후 잔상** — 경기 종료 시 ⓐ즉시 dismiss / ⓑ최종 스코어로 N분 유지 후 dismiss. → 추천 ⓑ(15분)
2. **앱 안 떠 있을 때 실시간성** — APNs Live Activity push(W3 ⓑ)는 서버 발송 빈도 = ⓐ매 이벤트(득점/카운트) / ⓑ주기(30초~1분). 매 이벤트는 배터리/비용↑. → 추천: 스코어·이닝 변화는 즉시, 단순 볼카운트는 주기 묶음
3. **시작 후 경기룸 나가면** — Activity 유지(경기 끝까지) vs 종료. → 추천 유지(잠금화면 목적상)
4. **여러 경기 동시** — 내 팀 경기 1개만 vs 여러 경기. → v1은 진입한 경기 1개(최근 진입 우선)
