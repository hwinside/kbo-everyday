...
- 자유 게시판 글 → **작성자 응원팀 컬러(`team_id`)**, 없으면 중립 다크 (하린아빠 2026-05-29 확정)
- 전체글에서는 각 글의 **원 보드 컨텍스트**를 그대로 따름 (팀글=팀, 선수글=선수, 자유글=작성자팀컬러/없으면 중립)
...
- **Q2. 긴 글 펼침**: ✅ 해결 — **3줄 후 '더 보기'**(피드 내 인라인 펼침, 상세 이동 없음) (하린아빠 2026-05-29 확정). S1 C카드에 반영됨.
- **Q3. 자유게시판 짧은 글 배경**: ✅ 해결 — **작성자 응원팀 컬러(`team_id`), 없으면 중립 다크** (하린아빠 2026-05-29 확정). S1 `PhotoFeed.tsx` 배경 로직에 반영됨.
- **Q4. 딥링크 리다이렉트 깊이**: ✅ 해결 — **`?post=ID` 스크롤+하이라이트**(공유 링크 UX 유지) (하린아빠 2026-05-29 확정). S4 슬라이스에서 구현.
...
## 12. S1 구현 전략 확정 (2026-05-29 PhotoFeed 정독)

`PhotoFeed.tsx`가 이미 **단일 피드의 거의 전부**를 구현 중 — 프로젝트 리스크 대폭 감소:
- 작성자 헤더 / 액션바(좋아요·댓글) / 캡션 더보기(`CaptionBlock` 2줄 클램프) / 플레이어태그 / 해시태그
- 미디어 캐러셀 + 핀치줌(회귀 다발 구간, 이미 안정화)
- **인라인 댓글: `CommentSheet`를 카드에서 직접 오픈** (상세 진입 X) → 댓글 아이콘 클릭 = (b), 입력창 = (c) 이미 동작
- 내부 `likedPosts` Set은 빈 상태로 시작 (배치 프리페치 없음)

→ **S1 = PhotoFeed를 통합 피드로 일반화**(신규 컴포넌트 fork 아님, 줌 로직 보존):
- 미디어 있는 post → 기존 캐러셀 카드 그대로 (카드 A)
- 미디어 없는 post → 캐러셀 자리에 **텍스트 카드 B/C** 분기 렌더. 본문 = title+content 합산. 짧으면(≤80자) 브랜드 배경 카드, 길면 3줄 클램프 + 더보기
- 빈 상태 문구 "아직 사진이…" → "아직 글이 없어요"로 일반화
→ **S2 = 배치 좋아요만 신규**: `useUnifiedFeed.likedIds`/`setPostLiked`를 PhotoFeed의 내부 Set 대신 주입. 댓글 트리거는 이미 됨.
