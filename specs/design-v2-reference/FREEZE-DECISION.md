...
### 🟡 조건부 GO
- *선수 Radar Chart* — "리그 평균 대비 해석 문장" 붙이는 조건에서만
- 구현 담당 태스크: T_radar_interpretation (아래 신규 태스크에 반영)
...
### G. 커뮤니티 (삼순이 P3 + 삼식이 P0)
- 내 팀 보드 *상단 고정* (팀팬 모드)
- "N" 뱃지 → `🔥 새 글` 명확화
- 핫토픽 1축 강조 (팀 게시판 정렬 칩 밀도 낮춤)
- 레벨 시스템 `lvl` 표시 → V1에 없으면 제거 (확인 필요)

### H. 선수 상세 (삼순이 P4 조건부 GO)
- Radar Chart 아래 *해석 문장 1~2줄*
- 포맷 예: `"리그 평균 대비 타율 +0.032, 홈런 -3, OPS +0.087. 공격형 3루수 유형."`
- AD/아이콘 탭/Radar 동시 노출 금지 → 순차 스크롤
...
### 미확인 질문 (하린아빠)
1. 레벨 시스템 V1 구현 여부 — 있으면 유지, 없으면 제거
2. NEUTRAL accent 브랜드 가이드 — 대체 색상 제안 필요 여부
3. 광고 슬롯 정책 — 위치/사이즈/광고사 (AdMob/AdSense/Google Ad Manager)
4. TabBar 5탭(홈=팀팬 커스텀) vs 4탭(경기=홈) 최종 결정
