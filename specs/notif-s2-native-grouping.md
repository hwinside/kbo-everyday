...
- **렌더**: 신규 클래스 `EventNotifications`(정적 헬퍼) 또는 `GameNotificationPlugin`에 `postEvent(ctx, gameId, eventId, title, body, path)` 추가. `NotificationCompat.Builder`로:
  ...
  - 개별 이벤트에 `setTimeoutAfter(6h)`로 **최후 보루 자동만료**(종료 신호 유실 시에도 무한 잔류 방지). 하린아빠 확정(2026-07-27): **6시간**. "지나간 알림 다 보존"이 우선이므로 timeout은 종료 정리(game_end cancel)의 백업일 뿐 — 6h는 경기 자연 길이+α 여유로 정상 종료이 먼저 지우는 것을 보장.
...
## 불확실 / 결정 필요 지점
1. ~~`setTimeoutAfter` 상한 값~~ → **해결(하린아빠 2026-07-27): 6시간**. 종료 정리 백업, 정상 종료이 먼저 cancel.
