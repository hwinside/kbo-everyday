...
현행 하드차단보다 **후퇴 금지**. V1부터 고신뢰군은 즉시 하드차단.

| 등급 | 대상 | V1 동작 |
|---|---|---|
| HARD | 명백한 우회 욕설(`ㅆ벌/시팔/시부럴/ㅅㄲ/쌔끼/야랄`, 의도적 `ㅗ`), 직접 위협(`죽어/닥쳐/멸종돼라` — 위협 문맥에 한함) | **완전 거부**(insert 자체 차단, raw 미보존) |
| SOFT | 문맥 의존어(`미친/ㅈㄴ/개-` 계열) | shadow(무동작 로깅) → 소프트 가림 순차 승격 |
| PASS+가중 | 경기력 야유(`못친다/2군보내라/방출`) | 통과, 신고 누적 시 가중치 |

## 2.1 HARD 판정 문법 & 반례 (blocker④)
