...
| 역할 | 토큰 | 용도 | 보정 |
|------|------|------|------|
| 아이덴티티 | `colorPrimary` | 배지, 로고, 팀명 텍스트 | 원색 그대로 |
| 배경/액션 | `getTeamBgColor()` | gradient, FAB, 전송 버튼, 하단탭 active, 헤더 border | luminance < 0.05 → `colorLight` fallback |
| 시스템 fallback | `#FF453A` | 팀 미지정 사용자 | 항상 고정 |

**어두운 팀 보정 (luminance < 0.05):**
| 팀 | colorPrimary (원색) | 보정 후 (colorLight) |
|----|----|----|
| 두산 | `#131230` | `#9BA8D4` |
| KT | `#000000` | `#E85050` |
| 롯데 | `#002856` | `#6BC4E8` |
...
**적용 범위:**
- ✅ 하단탭 active 컬러 → 지정팀 `getTeamBgColor`
- ✅ 전송 버튼 (팀 게시판) → 해당 게시판 팀컬러
- ✅ gradient header, FAB, 필터, 순위 하이라이트, 예측바, 뉴스 배지
- ✅ 헤더 border (1뎁스~2뎁스)
- ❌ 팀 배지/로고 → 원색(`colorPrimary`) 유지

**disabled 상태:** 동일 팀컬러 + `opacity-50`
...
|------|------|--------|-------------|------|
| 대제목 | 28px | Bold | 1.2 | |
| 1뎁스 타이틀 | 24px (text-2xl) | Bold | 1.3 | 홈/경기/커뮤니티/선수/순위 |
| 2뎁스 서브 | 18px (text-lg) | Semibold | 1.4 | 팀/선수/구장/게시판 상세 헤더 |
| 3뎁스 모달 | 16px (text-base) | Semibold | 1.4 | 댓글, 신고 등 |
| 본문 | 15px | Regular | 1.5 | |
| 캡션 | 13px | Regular | 1.4 | 사진 피드: line-clamp-2 + "더보기" |
...
└──────────────────────────────────────────┘
```
- 5탭 구성 (홈/경기/커뮤니티/선수/순위)
- 활성 탭: **지정팀 컬러** (`getTeamBgColor`) + filled 아이콘
- 팀 미지정: 시스템 accent (`#FF453A`)
- 비활성: text-secondary + outline 아이콘
- 글래스모피즘 배경 (`rgba(10,10,11,0.85)` + `backdrop-blur-xl`)
