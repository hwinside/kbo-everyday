...
## 9. UI 구현 — 초대 UI + 동적 OG (확정 2026-04-09 19:41 #product)

### 9.1 기존 유저: MY > 친구 초대 섹션
- 위치: `src/app/(main)/my/page.tsx` 내 ProfileCard 아래
- 구성요소:
  - 잔여 코드 수 표시
  - "초대코드 생성" 버튼 → 코드 생성 API 호출
  - 생성된 코드 + "공유하기" 버튼 (Web Share API, fallback: 클립보드 복사)
  - 공유 메시지: "크보팬에서 같이 야구 봐! ⚾ 초대코드: KBO-XXXXXX → keubo.fan/invite/KBO-XXXXXX"
  - 초대 현황 리스트: 초대한 사람 (활성화/대기 구분)
  - 개척자 뱃지 진행률 바 (activatedCount / 20)

### 9.2 신규 유저: 가입 플로우 초대코드 입력
- 위치: `ProfileSetupModal.tsx` — 팀 선택(step 2) 직후, 완료 전
- 이미 inviteCode state 존재 → step 3으로 "초대코드가 있나요?" 화면 추가
- 선택 입력 (건너뛰기 가능)
- 유효한 코드: "○○님의 초대로 가입!" 피드백
- 잘못된 코드: 에러 메시지, 재입력 또는 건너뛰기

### 9.3 예외 회수: MY에서 나중에 초대코드 등록
- `invited_by`가 NULL인 유저 한정, MY 페이지에 "초대코드 등록" 버튼 노출
- 1회만 사용 가능 (등록 후 버튼 사라짐)

### 9.4 딥링크 + 랜딩페이지
- 경로: `src/app/invite/[code]/page.tsx`
- `keubo.fan/invite/KBO-XXXXXX` → 코드 자동 채움
- 신규 유저: 가입 플로우로 이동 (코드 prefill)
- 기존 유저: "이미 가입된 계정입니다" 안내

### 9.5 동적 OG 이미지
- 경로: `src/app/invite/[code]/opengraph-image.tsx`
- `@vercel/og` (ImageResponse) 사용 — 기존 `src/app/opengraph-image.tsx` 패턴 참조
- 동적 요소: 초대자 닉네임만 (팀 로고 없음 — 다른 팀 팬 초대 케이스)
- OG title: "○○님이 크보팬에 초대했어요"
- OG description: "좋아하는 팀 고르고, 같이 경기 보고 놀아요"
- OG image: 크보팬 브랜드 다크 배경 + "○○님이 크보팬에 초대했어요 ⚾" 텍스트
- OG에 초대코드 자체는 넣지 않음 (카톡 캐시 이슈 방지)
- 닉네임 조회 실패 시 정적 fallback: "크보팬에 초대받았어요"
