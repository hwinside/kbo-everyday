...
승률 = 승 ÷ (승+패)로 계산합니다.'
      ...
      AND current_row.aliases IS NOT DISTINCT FROM ARRAY['draw','무']::text[] THEN
    UPDATE public.baseball_terms SET answer = 'KBO는 연장(정규시즌 12회)까지 동점이면 무승부입니다.
승률 계산에서 무승부는 승수에도 패수에도 넣지 않습니다.
승률 = 승 ÷ (승+패)로 계산합니다.', source_url = 'https://www.koreabaseball.com/Reference/Etc/GameRule.aspx', reviewed_at = '2026-07-30'::date WHERE term = '무승부';
  ELSE
...
KBO 정규시즌은 승부치기 없이 12회까지 진행하고, 그래도 동점이면 무승부입니다.', source_url = 'https://www.koreabaseball.com/Reference/Etc/GameRule.aspx', reviewed_at = '2026-07-30'::date WHERE term = '연장전';
  ELSE
