...
enum ChannelMigrationOrchestrator {
    ...
    /// - Parameters:
    ///   - enumerateCards: 이 game의 live·active 카드 marker 배열을 *호출 시점에 재-enumerate*
    ///     (레거시 nil). fetch 전 1회(직렬 구간 재검증) + fetch 후 1회 — plan/request/end는
    ///     반드시 fetch *후* fresh snapshot으로 계산한다(R6 blocker: fetch 중 사용자가 카드를
    ...
    static func reconcile(
        ...
        switch ChannelMigrationPolicy.recheck(
            ...
            legacyStillActive: !(await enumerateCards()).isEmpty
        ) {
        ...
        // R6 blocker — fetch를 await하는 동안 사용자가 카드를 dismiss했거나 카드가 final/ended
