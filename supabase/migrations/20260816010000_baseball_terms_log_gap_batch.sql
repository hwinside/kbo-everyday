...
-- 🔴 삼순 2026-08-16 4·5차 NO-GO ④: 팬이 `3피트룰` 이라 부르는 것은 KBO 공식야구규칙의
--   **서로 다른 두 조항**이다 — 5.09(a)(8) 1루 러닝 레인 방해, 5.09(b)(1) 태그 회피 주로 이탈.
--   4차 판에서 두 조항을 나누기는 했으나 **양쪽 경계를 모두 틀리게 적었다**(삼순 5차):
--     · 5.09(a)(8): "파울 라인 바깥 레인 안으로 달려야" 로 단정했는데, KBO 는 3피트 레인
--       (라인 위 포함) **또는 파울 라인 왼쪽 내야 흙**을 허용한다(내야 흙 인정은 KBO 한정).
--       그리고 벗어난 것 자체가 아니라 **심판이 방해를 인정했을 때** 아웃이다.
--     · 5.09(b)(1): "3피트를 넘게" → 규칙 문면은 **3피트(91.4cm) 이상**이고, "그 자체로
--       아웃" 도 틀렸다 — **타구를 처리하는 야수를 방해하지 않으려고 벗어난 것은 무방**하다.
('3피트룰', ARRAY['3피트 룰','쓰리피트','three foot rule','3피트 라인','스리피트'],
 '3피트룰은 서로 다른 두 규정을 함께 부르는 말입니다.
타자 주자는 본루와 1루 사이 후반부를 달릴 때 두 발이 3피트 레인 안이나 파울 라인 왼쪽 내야 흙 위에 있어야 하고, 3피트 라인 바깥이나 내야 잔디를 달려 1루 송구나 포구를 방해했다고 심판이 인정하면 아웃입니다.
주자가 태그당하지 않으려고 베이스를 잇는 직선에서 3피트 이상 벗어나 달렸을 때도 아웃이지만, 타구를 처리하는 야수를 방해하지 않으려고 벗어난 것은 괜찮습니다.',
