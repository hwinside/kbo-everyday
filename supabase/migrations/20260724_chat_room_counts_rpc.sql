...
RETURNS TABLE (total_count BIGINT, home_count BIGINT, away_count BIGINT)
LANGUAGE sql
STABLE
...
AS $$
  SELECT
    ...
    COUNT(*) FILTER (WHERE p.team_id = p_home_team_id)::BIGINT AS home_count,
    COUNT(*) FILTER (WHERE p.team_id = p_away_team_id)::BIGINT AS away_count
...
--   -- 기대: 기존 head 3발 결과와 동일한 total/home/away 1 row.
