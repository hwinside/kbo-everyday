-- 이름 오타 되묻기 감사 식별자 — `match_path='name_suggest'`
--
...
--
-- 로스터 881명에 `임창규` 는 **없다**(`임찬규` kboId 61101, LG 만 있다). 근거가 하나도
-- 없는 상태에서 generic LLM 이 받아 **존재하지 않는 사람을 실존으로 만들고** 소속과
-- 위상까지 붙였다. 수치 환각보다 나쁘다 — 유저는 틀렸다는 걸 알 방법이 없다.
--
-- ── 왜 기존 라벨에 접지 않는가 ───────────────────────────────────────────────
--
...
-- 이미 `scope_guide` 에서 틀린 것으로 판명됐다.
-- (`team_rag`·`news_rag` 를 `rag` 에서, `scope_guide` 를 `ack` 에서 분리한 것과 같은 축.)
--
-- ⚠️ `name_unknown` 은 **넣지 않는다.** 초안에는 있었는데(제안할 이웃이 없는 경우),
--   최종 구현이 실측 오타 alias map 으로 좁혀지면서 그 상태 자체가 사라졌다 —
--   map 에 있으면 교정 대상이 항상 하나다. 도달 불가능한 값을 CHECK 에 남기면
--   "이 경로가 살아 있다" 는 거짓 신호가 되므로 뺀다.
--
