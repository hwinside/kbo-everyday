...
ALTER TABLE public.genius_question_logs
  ADD CONSTRAINT genius_question_logs_match_path_check CHECK (
    match_path IN (
      ...
      'news_rag',
      'scope_guide',
      -- 로스터에 없는 실명을 받아 생성 없이 이름을 되물은 경로(제안할 이웃이 정확히 1명).
      'name_suggest',
      -- 같은 상황인데 **제안할 이웃이 없어** 모른다고만 답한 경로.
      --
      -- ⚠️ 왜 또 쪼개는가 (삼순 2026-08-08 조건 ③). 두 경로의 감사 질문이 다르다:
      --     name_suggest → "제안이 몇 건 나갔고 그중 오제안은 몇 건인가"
      --     name_unknown → "모른다로 끝난 게 몇 건인가" = 커버리지 구멍의 크기
      --   한 칸에 넣으면 name_unknown 이 압도적으로 많아(이웃 없는 이름이 더 흔하다)
      --   오제안율이 실제보다 낮게 보인다 — 안전해 보이는 방향으로 틀린다.
      'name_unknown'
    )
  );
