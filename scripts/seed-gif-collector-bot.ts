/**
 * 움짤콜렉터 봇 계정 seed (idempotent).
 *
 * Usage:
 *   npx tsx scripts/seed-gif-collector-bot.ts
 *
 * Env:
 *   SUPABASE_URL or NEXT_PUBLIC_SUPABASE_URL
 *   SUPABASE_SERVICE_ROLE_KEY (필수)
 *
 * 출력의 GIF_COLLECTOR_BOT_ID=<uuid>를 secret manager에 저장.
 * 폴러/매칭 엔진이 게시 시 author_id로 이 UUID 사용.
 */
...
const BOT_EMAIL = "gif-collector-bot@keubo.fan";
const BOT_NICKNAME = "움짤콜렉터";
...
async function main() {
  // src/app/api/auth/naver/callback/route.ts와 동일 패턴.
  ...
  if (!user) {
    const { data, error } = await supa.auth.admin.createUser({
      email: BOT_EMAIL,
      email_confirm: true,
      user_metadata: { is_bot: true, bot_kind: "gif_collector" },
    });
    if (error) throw error;
    user = data.user!;
  ...
  console.log(`\nGIF_COLLECTOR_BOT_ID=${user.id}`);
  console.log("위 ID를 secret manager에 저장 후 폴러/매칭 엔진에 주입.");
}
