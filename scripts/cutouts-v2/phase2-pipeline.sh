...
# 투명 PNG의 alpha 합성 이미지는 sleeve/button false positive가 발생하므로
# raw PNG (gray 배경, 불투명)에서 face detect 후 좌표를 투명 PNG에 적용
...
if raw_img is not None:
    ...
    # raw가 없으면 투명 PNG fallback (기존 방식)
    alpha = img[:, :, 3]
    rgb = img[:, :, :3]
...
for stage, (sf, mn, ms) in enumerate([
    (1.1, 5, (100, 100)),    # Stage 1: strict
    (1.05, 3, (80, 80)),     # Stage 2: moderate
    (1.03, 2, (50, 50)),     # Stage 3: permissive (small faces)
]):
    faces = face_cascade.detectMultiScale(raw_gray, scaleFactor=sf, minNeighbors=mn, minSize=ms)
    # 상단 2/3 안에 있는 후보만 인정 (팔뚝/허리 false positive 차단)
    ...
    if upper:
        ...
        if largest[2] >= MIN_FACE_PX:
            ...
            face = largest  # 작지만 일단 후보로 보관, 다음 stage에서 더 큰 거 시도
...
# 정상 얼굴은 w:h ≈ 1:1 ~ 1:1.4 범위 (정면 + 모자 포함)
# 유니폼 로고/단추 false positive는 극단적 비율
