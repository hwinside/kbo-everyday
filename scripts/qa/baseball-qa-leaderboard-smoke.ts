...
check("P0: 순위 표지 형태 — 목록에 없는 새 지표도 순위를 물으면 hold", () => {
  for (const q of [
    ...
    "역대 견제사 최다 누구야?",
    "통산 끝내기 선두 누구야?",
    // 삼순 7차 P0 exact — 서수 없이 **수량 비교**로 묻는 형태 (실사용 다수)
    "통산 폭투 누가 제일 많아?",
    "역대 견제사 누가 많아?",
    "통산 끝내기 상위 10명 알려줘",
    "역대 몸에맞는공 누가 많이 맞았어?",
    "통산 다승 누가 제일 많아?",       // 삼순 5차 alias 축 — 표지 축으로도 닫힌다
    "역대 이닝 누가 많이 던졌어?",
  ]) {
    assert.equal(routeQuestion(q, [], PLAYERS), "history_hold", q);
  }
});
// ⚠️ 삼순 9차 P0 — bare `많` 과차단. 시점어가 있어도 **리더보드가 아닌 서술형**이면
// 답변 범위를 깎으면 안 된다. 이 축이 `많` 을 완전문법으로만 인정하는 이유의 반대편이다.
check("P0 무회귀: 시점어 + `많` 이 있어도 리더보드형이 아니면 LLM 범위 판정", () => {
  for (const q of [
...
check("무회귀: 지표도 순위 표지도 없는 주관 평가는 hold 가 아니라 LLM 범위 판정", () => {
  // ⚠️ `가장`·`제일` 을 단독 표지로 넣으면 이 축이 전부 hold 로 끌려와 답변 범위가 좁아진다.
  // `많` 한 글자로만 닫는 이유의 반대편 안전핀이다.
  for (const q of [
    "역대 최고의 타자는 누구야?",
    "역대 가장 멋진 선수 누구야?",
    "역대 제일 인상 깊은 선수 누구야?",
