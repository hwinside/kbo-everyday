...
async function openUi(a) {
  ...
  const bypass = process.env.VERCEL_BYPASS
    ...
    : "";
  await page.goto(`${BASE}/games/${GAME_ID}${bypass}`, { waitUntil: "domcontentloaded", timeout: 60000 });
  a.context = context;
  a.page = page;
  return page;
}
...
async function waitVisibleNoReload(a, text, timeoutMs = 30000) {
  ...
  try {
    ...
    console.log(`    [latency] ${a.label} 수신까지 ${ms}ms (wait 시작 기준)`);
    globalThis.__latencies = globalThis.__latencies || [];
    globalThis.__latencies.push(ms);
    return true;
  } catch { return false; }
}
