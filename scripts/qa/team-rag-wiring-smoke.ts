...
async function run(): Promise<void> {
...

  {
    ...
    for (const answer of [
      ...
      // 축18: **미등재 어미 활용** — 11라운드 삼순 지적. 어미 첫 음절 allowlist 도
      //   결국 열거라 목록 밖 활용이 우회로가 됐다. 파서는 서술격 **어간**(이/인/입/예/였)
      //   만 보고 그 뒤 활용은 아예 보지 않으므로 새 활용이 생겨도 닫혀 있다.
      "LG 트윈스의 우승은 열인데요.",
      "LG 트윈스의 우승은 팔이면 많아요.",
      "LG 트윈스의 관중은 십만이며 늘었어요.",
      // 축19: **조사 결합** — `이나`·`까지`·`쯤`. 조사는 닫힌 품사라 열거가 성립한다.
      "LG 트윈스의 관중은 십만이나 돼요.",
      "LG 트윈스의 관중은 십만까지 늘었어요.",
      "LG 트윈스의 상금은 백만쯤이에요.",
      // 축20: **큰 자리수사 + 야구 수량명사**.
      //   ⚠️ 종전 fixture 는 `만타석` 이었는데 **한국어에서 안 쓰는 말**이다
      //   (하린아빠 2026-08-07 지적 → corpus 전량 실측: `만타석`·`만경기`·`만안타`·
      //    `만홈런` 전부 0건, 실사용은 `만 경기` 504·`만 안타` 245·`만 홈런` 159 로 **띄어 쓴다**).
      //   존재하지 않는 표현으로 게이트를 세우면 실제로는 안 오는 입력을 막느라
      //   사전만 넓어지고, 그 대가로 `만루` 같은 야구 어휘가 죽는다.
