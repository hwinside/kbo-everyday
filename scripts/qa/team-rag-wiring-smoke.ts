...
async function run(): Promise<void> {
  ...
  {
    // ④-5 **유일한 결정론 수치 가드** — 유니코드 숫자 문자.
    //
    //   `/\d/` 는 ASCII 만 봐서 아래가 전부 통과했다(실측). `\p{N}` 으로 통째로 닫는다.
    //   이 축을 남긴 기준: **반대가설을 만들 수 없다.** `1990`·`Ⅲ`·`⑧` 이 숫자가
    //   아닌 한국어 문맥은 없다. 자연어 판단이 아니라 문자 존재 확인이다.
    for (const answer of [
      "삼성 라이온즈는 1985년에 통합 우승했어요.",       // 아라비아
      "삼성 라이온즈는 １９８５년에 통합 우승했어요.",     // 전각
      "삼성 라이온즈는 Ⅲ회 우승했어요.",                  // 로마
      "삼성 라이온즈는 ⑧회 우승했어요.",                  // 원문자
    ]) {
      const { deps, logs } = makeDeps({
        callTeamRagLlm: async () => ({
  ...
  {
    // ④-5b 🔴 **한글 수사는 코드가 막지 않는다** — 이 게이트가 그 계약을 고정한다.
    //
    //   2026-08-07 하린아빠 지시로 한글 수사 파서(224줄 + 사전 190항목)를 삭제했다.
    //   "룰베이스 가드는 최소화하고 LLM 판단에 맡기기로 했는데 다시 예전 방향으로
    //    가고 있다" · "이런 식이면 앞으로도 계속 로직만 누더기로 추가할 것 같다"
    //
    //   무슨 일이 있었나: `여덟 번`·`열인데요`·`십만까지` 를 코드로 막으려다 12라운드를
    //   태웠다. 매 라운드가 같은 모양이었다 — 반대가설 1개 → 규칙 1개 → 야구 어휘
    //   과차단 → 규칙 좁힘 → 새 반대가설. 죽은 어휘가 하필 전부 우리 도메인이었다.
    //
    //   ⚠️ 아래 fixture 는 **두 가지를 동시에** 고정한다:
    //     ① 한글 수사가 섞인 답변도 이제 코드는 통과시킨다(막는 건 프롬프트 몫)
    //     ② 누군가 한글 수사 파서를 **다시 추가하면 여기가 RED 로 막는다**
    //   즉 이건 단순 통과 목록이 아니라 **아키텍처 원칙을 지키는 게이트**다.
    //
    //   ⚠️ 여기 항목을 "차단되어야 한다"로 뒤집지 마라. 그러려면 파서가 필요하고,
    //     그 방향은 이미 12라운드 동안 수렴하지 않는다는 것이 실측으로 확인됐다.
...

  {
    ...
    //   🔴 2026-08-07 계약 변경: 프롬프트와 가드는 **같은 것을 막지 않는다.**
    //     종전에는 "프롬프트 금지 예시를 가드가 전부 잡아야 한다"였고, 그 단언이
    //     한글 수사 파서를 12라운드 동안 키운 압력이었다(프롬프트가 `여덟 번` 을
    //     금지하니 가드도 잡아야 한다 → 규칙 추가 → 과차단 → 반례 → 반복).
    //
    //     이제는 **층을 나눈다**:
    //       · 프롬프트(1차) = 생성 자체를 금지한다. 한글 수사 포함 전부.
    //       · 코드 가드(2차) = 유니코드 숫자 문자만. 반대가설이 없는 것만 맡는다.
    //     따라서 금지 예시를 두 갈래로 갈라 **각자 제 몫만** 검증한다.
    const digitExamples = bannedExamples.filter((word) => hasNumericCharacter(word));
    const koreanOnlyExamples = bannedExamples.filter((word) => !hasNumericCharacter(word));

    //   축1-a: 숫자 문자가 든 금지 예시는 **코드 가드가** 잡아야 한다.
    assert.ok(digitExamples.length >= 2,
      `숫자 금지 예시를 못 뽑았다: ${JSON.stringify(bannedExamples)}`);
    for (const example of digitExamples) {
      assert.ok(hasNumericCharacter(example),
        `프롬프트가 금지한 숫자 표현을 가드가 통과시킨다: ${example}`);
    }

    //   축1-b: 한글 수사 금지 예시는 **프롬프트에 남아 있어야 하되**, 코드 가드는
    //     잡지 않는 것이 정상이다. 여기서 "잡힌다"면 파서가 되살아났다는 뜻이다.
    assert.ok(koreanOnlyExamples.length >= 1,
      `프롬프트에서 한글 수사 금지 지시가 사라졌다(1차 방어 소실): ${JSON.stringify(bannedExamples)}`);
    for (const example of koreanOnlyExamples) {
      assert.ok(!hasNumericCharacter(example),
        `한글 수사를 코드 가드가 막고 있다(파서 재추가): ${example}`);
    }
    ...
    assert.ok(hasNumericCharacter(before),
      `재작성 예시의 '고치기 전' 을 가드가 통과시킨다: ${before}`);
    assert.ok(!hasNumericCharacter(after),
      `재작성 예시의 '고친 후' 를 가드가 차단한다(모범답이 폐기됨): ${after}`);

    ok(`프롬프트↔가드 층 분리 — 숫자 ${digitExamples.length}종은 가드가, 한글수사 ${koreanOnlyExamples.length}종은 프롬프트가 담당 + 재작성 모범답 통과`);
  }
