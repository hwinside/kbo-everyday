...
const FAQ_FACTS: Partial<Record<ProductFeatureKey, readonly string[]>> = {
  ...
  "잠금화면중계": ["iOS 18 이상", "Android 16 이상", "마이페이지 > 잠금화면 > 잠금화면 실시간 중계", "경기 시작 30분 전부터 종료까지", "잠금화면 카드 다시 표시", "일반 잠금화면 카드"],
  "최애선수": ["마이페이지 > 최애 선수", "최대 5명", "새 구단의 최애선수를 다시 선택"],
  "최애팀": ["새 구단의 최애선수를 다시 선택"],
};

/**
 * ⑤-b 문구가 가리키는 UI 라벨 — src 의 `.tsx` 에 문자열 그대로 실재해야 한다("실제 UI 진입", 삼순 ③).
