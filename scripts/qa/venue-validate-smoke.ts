...
(async () => {
  ...
  // transcode-videos.mjs 성공 경로 update 조건 모사:
  ...
  // catch(예외) 경로 모사:
  //   active: .eq("status","active").eq("needs_transcode",true) + payload에 status 없음
  //   pending: .eq("status","pending") 만 (needs_transcode CAS 없음) + payload에 status 있음
  function simulateCatchCas(opts: {
    rowStatus: string;
    rowNeedsTranscode: boolean;
    isPending: boolean;
  }): { rowsAffected: number; statusInPayload: boolean } {
    if (opts.isPending) {
      // pending catch: status CAS만, needs_transcode 미체크
      return { rowsAffected: opts.rowStatus === "pending" ? 1 : 0, statusInPayload: true };
    } else {
      // active catch: status + needs_transcode CAS 둘 다 필요, payload에 status 없음
      const rowsAffected = (opts.rowStatus === "active" && opts.rowNeedsTranscode) ? 1 : 0;
      return { rowsAffected, statusInPayload: false };
    }
  }

  console.log("[P0 #2 transcode worker resurrect 방지 CAS 불변식 — 성공 경로]");
  {
  ...
  console.log("[P0 #2 transcode worker resurrect 방지 CAS 불변식 — catch(예외) 경로]");
  {
    // active catch: 처리 중 concurrent removed → 0-row skip, status 부활 없음
    const r = simulateCatchCas({ rowStatus: "removed", rowNeedsTranscode: true, isPending: false });
    ok("catch active — concurrent removed → 0-row(resurrect 금지)", r.rowsAffected === 0);
    ok("catch active — status 페이로드 미포함(active 재기록 금지)", r.statusInPayload === false);
  }
  {
    // active catch: 이미 완료된 행(needs_transcode=false) → 0-row skip
    const r = simulateCatchCas({ rowStatus: "active", rowNeedsTranscode: false, isPending: false });
    ok("catch active — needs_transcode=false 이미 완료 → 0-row skip", r.rowsAffected === 0);
  }
  {
    // active catch: 여전히 active+needs_transcode=true → 1-row, attempts만 증가
    const r = simulateCatchCas({ rowStatus: "active", rowNeedsTranscode: true, isPending: false });
    ok("catch active — 정상 active → 1-row(attempts 증가, status 유지)", r.rowsAffected === 1 && r.statusInPayload === false);
  }
  {
    // pending catch: 즉시경로가 이미 active로 승격 → 0-row skip(덮어쓰기 금지)
    const r = simulateCatchCas({ rowStatus: "active", rowNeedsTranscode: false, isPending: true });
    ok("catch pending — 즉시경로 선점(active) → 0-row skip(pending 재기록 금지)", r.rowsAffected === 0);
  }
  {
    // pending catch: 여전히 pending → 1-row, nextStatus(pending/removed) 기록
    const r = simulateCatchCas({ rowStatus: "pending", rowNeedsTranscode: true, isPending: true });
    ok("catch pending — 여전히 pending → 1-row + status 페이로드 포함", r.rowsAffected === 1 && r.statusInPayload === true);
  }
