...
const MUTATIONS = [
  {
    ...
    file: SEASON,
    re: /  if \(UNTRUSTED_VALUE_ASK\.test\(spaced\)\) return true;/,
  ...
  },
  {
    ...
    file: SEASON,
    re: /  if \(playerBound && !UNTRUSTED_DEFINITION_ASK\.test\(spaced\)\) return true;/,
    to: "  // [mutant M2] bare query 축 제거",
  },
  {
  ...
  },
  {
    ...
    file: SEASON,
    re: /  if \(playerBound && !UNTRUSTED_DEFINITION_ASK\.test\(spaced\)\) return true;/,
    to: "  if (playerBound) return true; // [mutant M4]",
  },
  {
