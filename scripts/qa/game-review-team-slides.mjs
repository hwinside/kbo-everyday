...
assert.match(document.body.textContent,/2 \/ 25/);
...
`, resolveDir: process.cwd(), loader: 'tsx' }, bundle: true, platform: 'node', format: 'cjs', packages: 'external', write: false,
  ...
  const entry = join(dir, 'test.cjs'); await writeFile(entry, result.outputFiles[0].text);
