...
const MUTATIONS = [
  ...
  {
    name: "m17 구조 잔여검사 제거 — 두 시즌/열한 번째/마지막 시즌이 career total 로 축소 (삼순 9차 재현)",
    file: SEASON,
    from: "if (unitLeft) {",
    to: "if (false) {",
  ...
  {
    name: "m16 series 표지 소거 제거 — 데뷔 후 시즌별 추이가 other 로 과차단 (삼순 9차 과차단 방지)",
    file: SEASON,
    from: "after = after.replace(/(?:시즌별|연도별|년도별|경기별|월별|추이)/g,",
    to: "after = after.replace(/(?:__never__)/g,",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
    name: "m15 시간 단위 검출 제거 — 입단 후 첫해/첫 시즌이 career total 로 축소 (삼순 6차 재현)",
    file: SEASON,
    from: "const unitLeft = /(?:시즌|경기|타석|등판|년도?)|(?:첫|째|번째|마지막)해/.test(after);",
