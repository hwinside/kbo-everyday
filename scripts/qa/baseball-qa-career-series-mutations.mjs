...
const MUTATIONS = [
  ...
  {
    name: "m17 첫째 이외 서수 일반화 제거 — 두 번째/둘째/2번째가 career total 로 축소 (삼순 8차 재현)",
    file: SEASON,
    from: "(?:첫(?:번째|째)?|(?:두|둘|세|셋|네|넷|다섯|여섯|일곱|여덟|아홉|열)(?:번째|째)|\\d{1,2}번째)",
    to: "첫(?:번째)?",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
    ...
    file: SEASON,
    from: "/(?:데뷔|입단)[^]{0,12}?(?:첫(?:번째|째)?|(?:두|둘|세|셋|네|넷|다섯|여섯|일곱|여덟|아홉|열)(?:번째|째)|\\d{1,2}번째)(?:해|시즌|경기|타석|등판|년도?)/",
  ...
  },
  {
    ...
    file: SEASON,
    from: '} else if (/(?:데뷔|입단)[^]{0,12}?(?:첫(?:번째|째)?|(?:두|둘|세|셋|네|넷|다섯|여섯|일곱|여덟|아홉|열)(?:번째|째)|\\d{1,2}번째)(?:해|시즌|경기|타석|등판|년도?)/.test(rest)) {',
    to: "} else if (false) {",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
