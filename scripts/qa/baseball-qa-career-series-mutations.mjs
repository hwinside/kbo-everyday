...
const MUTATIONS = [
  ...
  {
    name: "m17 literal 계약 제거 — 데뷔 이래/후 가 full-origin 으로 축소 (frozen contract 재현)",
    file: SEASON,
    from: "if (CANONICAL_DEBUT_SERIES.test(rest)) {",
    to: "if (true) {",
  ...
  {
    name: "m16 cutoff 옵션 제거 — 캡처 exact(현재까지)가 other 로 과차단 (양성 회귀 재현)",
    file: SEASON,
    from: "(?:시점|초)?(?:이래|이후|부터|후)(?:(?:현재|지금|올해|오늘)까지)?",
    to: "(?:시점|초)?(?:이래|이후|부터|후)",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
    name: "m15 인접 요구 완화 — canonical 과 series 사이 임의 문자 허용 시 3년 연도별이 통과",
    file: SEASON,
    from: "(?:(?:현재|지금|올해|오늘)까지)?(?:연도별|년도별|시즌별)",
    to: "(?:(?:현재|지금|올해|오늘)까지)?[^]*?(?:연도별|년도별|시즌별)",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
