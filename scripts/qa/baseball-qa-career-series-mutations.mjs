...
const MUTATIONS = [
  ...
  {
    name: "m17 A′ 폐쇄집합 조건 제거 — 데뷔 이래/후 가 통산으로 축소 (삼순 10차 계약 재현)",
    file: SEASON,
    from: "if ((seriesReq || careerReq) && !unitLeft) {",
    to: "if (true) {",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
    name: "m18 월별/경기별 fail-close 제거 — 월단위 시계열이 연도별로 둔갑 (삼순 10차)",
    file: SEASON,
    from: 'if (/월별|경기별/.test(scan.spaced)) return { kind: "unsupported_season" };',
    to: "",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
    name: "m16 probe series 소거 제거 — 데뷔 후 시즌별 추이가 other 로 과차단 (과차단 방지)",
    file: SEASON,
    from: 'let probe = rest.replace(/(?:시즌별|연도별|년도별|추이)/g, "\\u0000");',
    to: 'let probe = rest.replace(/(?:__never__)/g, "\\u0000");',
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
    name: "m15 bounded 잔여검사 제거 — 데뷔 후 3년 추이가 전체 시계열로 둔갑 (삼순 10차)",
    file: SEASON,
    from: "const unitLeft = /(?:시즌|경기|타석|등판|년도?)|(?:첫|째|번째|마지막)해/.test(probe);",
    to: "const unitLeft = false;",
    smoke: "scripts/qa/baseball-qa-career-series-smoke.ts",
  },
  {
