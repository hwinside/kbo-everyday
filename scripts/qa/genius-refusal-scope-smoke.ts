...
check("과차단 0 — 메타어 **앞**의 수식어는 조사가 아니다 (`가을야구`)", () => {
  // 2026-08-16 운영 로그 전수조사 실측 결함.
  //
  // 한국어 조사는 내용어 **뒤**에 붙는데, 종전 구현은 잔여물의 위치를 보지 않았다:
  //   `가을야구` → 메타어 `야구` 제거 → 잔여 `가을` → `을` 떼고 `가` 떼면 빈 문자열
  //             → "조사만 남았다" → 범위 안내문
  // 즉 포스트시즌을 물은 유저가 "제가 답할 수 있는 범위는…" 을 받았다(로그 2회 재현).
  //
  // ⚠️ `겨울야구`·`동네야구` 는 우연히 조사로 안 쪼개져 멀쩡했다 — **우연이 계약을
  //   대신하고 있었다**. 그래서 우연히 통과하던 것들도 같이 고정한다.
  for (const q of [
    "가을야구",      // 붙여쓰기 — 로그 원문
    "가을 야구",     // 띄어쓰기 — 붙여쓴 쪽만 고치면 여기서 RED
    "가을야구가 뭐야",
    "가을야구 언제 시작해?",
    // 우연히 통과하던 동류 — 조사로 안 쪼개졌을 뿐 같은 구조다.
    "겨울야구", "동네야구", "사회인야구",
  ]) {
    assert.equal(isScopeAskPhrase(q), false, `메타어 앞 수식어를 조사로 분해했다: ${q}`);
  }
});

check("무회귀 — 메타어 **뒤**의 조사는 계속 조사다", () => {
  // 위 위치 조건이 조사 처리 자체를 죽이면 그게 더 큰 회귀다(범위 되묻기가 통째로 샌다).
  for (const q of ["야구 룰은 뭔가있어?", "야구룰은", "규칙은 뭐가 있어", "야구 용어는?"]) {
    assert.equal(isScopeAskPhrase(q), true, `메타어 뒤 조사 처리가 죽었다: ${q}`);
  }
});

check("무회귀 — **완전 무공백** 범위 질문은 여전히 범위 되묻기다 (삼순 2026-08-17 P0)", () => {
  // 🔴 1차 구현은 위치를 `raw.endsWith(residue)` 로 — 즉 **원문의 끝**으로 쟀다가
  //   이 부류를 통째로 깨뜨렸다. 띄어쓰기가 없으면 조사 뒤에 서술어까지 붙어 오기 때문이다:
  //     `야구룰은뭐가있어` → 꺼풀까지 떼면 잔여 `은` 인데 원문은 `있어` 로 끝난다 → false
  //   그래서 위치 판정을 **꺼풀 제거 후 형태**(메타어 자리 마커 기준 머리/꼬리)로 옮겼고,
  //   그 사실을 이 예제들로 고정한다. 이건 새 기능이 아니라 **기존 계약**이다.
  for (const q of [
    "야구룰은뭐가있어",      // 삼순 지적 원문
    "야구규칙은뭔가있어",    // 삼순 지적 원문
    "야구룰은뭐야",
    "프로야구규칙은뭐가있어", // 긴-메타어 우선 제거와 같이 성립해야 한다
  ]) {
    assert.equal(isScopeAskPhrase(q), true, `무공백 범위 질문을 놓쳤다: ${q}`);
  }
});
