...
const MUTATIONS = [
  ...
  },
  {
    ...
    file: CAREER_LEADERBOARD,
    from: "  if (hasCareerRankMarker(normalized)) return true;",
    to: "",
    smoke: "scripts/qa/baseball-qa-leaderboard-smoke.ts",
  },
  {
  ...
  {
    name: "m1e2 수량 비교 표지 제거 — `누가 제일 많아?` 형태가 generic LLM 으로 샌다",
    file: CAREER_LEADERBOARD,
    from: "  return CAREER_RANK_ORDINAL.test(normalized) || CAREER_RANK_QUANTITY.test(normalized);",
    to: "  return CAREER_RANK_ORDINAL.test(normalized);",
    smoke: "scripts/qa/baseball-qa-leaderboard-smoke.ts",
  },
  {
    name: "m1e3 범위 표지(`상위 N명`) 제거 — `상위 10명` 이 서수 없이 샌다",
    file: CAREER_LEADERBOARD,
    from: "|상위\\s*\\d+\\s*(명|인)/;",
    to: "/;",
    smoke: "scripts/qa/baseball-qa-leaderboard-smoke.ts",
  ...
  {
    name: "m1e4 주관 평가까지 표지 확대(`가장|제일`) — 답변 범위가 좁아지는 반대편 회귀",
    file: CAREER_LEADERBOARD,
    from: "const CAREER_RANK_ORDINAL = /\\d+\\s*위|최다|선두",
    to: "const CAREER_RANK_ORDINAL = /가장|제일|\\d+\\s*위|최다|선두",
    smoke: "scripts/qa/baseball-qa-leaderboard-smoke.ts",
  },
  {
    name: "m1e5 `많` 완전문법 해제(bare 매칭 복원) — 서술형 질문까지 hold 로 과차단",
    file: CAREER_LEADERBOARD,
    from: "const CAREER_RANK_QUANTITY = /누가.{0,6}많|많은\\s*(선수|타자|투수|사람|이)|상위\\s*\\d+\\s*(명|인)/;",
    to: "const CAREER_RANK_QUANTITY = /많|상위/;",
    smoke: "scripts/qa/baseball-qa-leaderboard-smoke.ts",
  },
  {
