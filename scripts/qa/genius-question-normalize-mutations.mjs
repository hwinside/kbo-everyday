...
const mutations = [
  ...
  {
    name: "M8 Tier B 로스터 엔티티 불변 가드 제거 (선수 치환 통과)",
    find: "if (qRoster.size !== cRoster.size || [...qRoster].some((k) => !cRoster.has(k))) return rejected;",
    replace: "if (false) return rejected;",
  },
  {
    name: "M9 Tier B 폐쇄집합 착지 가드 제거 (임의 재작문 통과)",
    find: "if (!landsInClosedSet) return rejected;",
    replace: "if (false) return rejected;",
  },
  {
    name: "M10 관측 status 기록 제거 (미호출/거절/오류 구분 소실)",
    find: "normalizeStatus: normStatus,",
    replace: "",
  },
  {
    name: "M11 Tier A 조건 완화 (문자 구성이 달라도 surface 수용 — 엔티티 가드 우회)",
